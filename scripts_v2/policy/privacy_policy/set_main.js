function set_main() {
  ...
  function getPathLang() {
    ...
    return supported.includes(parts[0]) ? resolveLang(parts[0]) : '';
  }
  ...
  function getPreferredLang() {
    ...
    if (winLang) return resolveLang(winLang);
    try {
  ...
  function syncDocumentLangAttrs(lang) {
    ...
    if (document.documentElement.getAttribute('lang') !== resolved) document.documentElement.setAttribute('lang', resolved);
    if (document.documentElement.getAttribute('dir') !== nextDir) document.documentElement.setAttribute('dir', nextDir);
    window.__STATKISS_LANG__ = resolved;
    const root = document.getElementById('div_main');
    if (root) {
      if (root.getAttribute('data-policy-slug') !== POLICY_SLUG) root.setAttribute('data-policy-slug', POLICY_SLUG);
      if (root.getAttribute('data-policy-lang') !== resolved) root.setAttribute('data-policy-lang', resolved);
      if (root.getAttribute('data-policy-dynamic') !== '1') root.setAttribute('data-policy-dynamic', '1');
  ...
  function updateHeadFromDocument(lang, documentData) {
    ...
    const absolute = window.location.origin + '/' + resolved + '/privacy-policy/';
    ...
    if (canonical.getAttribute('href') !== absolute) canonical.setAttribute('href', absolute);
  ...
  function Div_main() {
    ...
    useEffect(() => {
      ...
      const onStorage = (e) => {
        if (!e) return;
        if (e.key === getLangKey() || e.key === 'statkiss_lang') applyLang(e.newValue || null);
      };
      ...
      const onPopState = () => applyLang(getPathLang() || getPreferredLang());
    ...
    useEffect(() => {
      updateHeadFromDocument(lang, documentData || {});
    }, [lang, documentData]);
    ...
    return (
      <section className="policy-surface">
        <div className="mx-auto max-w-screen-xl px-4 py-10 sm:py-14 lg:px-6">
          ...
          <div className="policy-meta-card mx-auto mt-8 grid max-w-screen-lg gap-4 rounded-3xl border px-5 py-5 text-center shadow-sm sm:grid-cols-2 sm:px-6" dir={dir}>
