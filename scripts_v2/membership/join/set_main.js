function set_main() {
    ...
    function getMembershipBasePath() {
        const parts = String(window.location.pathname || '/').split('/').filter(Boolean);
        const membershipIndex = parts.indexOf('membership');
        if (membershipIndex >= 0) {
            return '/' + parts.slice(0, membershipIndex + 1).join('/') + '/';
        }
        const docLang = String(document.documentElement.getAttribute('lang') || '').trim();
        if (docLang) {
            return '/' + docLang + '/membership/';
        }
        return '/membership/';
    }

    function buildMembershipAjaxUrl(endpoint) {
        const basePath = getMembershipBasePath();
    ...
    function buildAccountPath() {
        ...
        const membershipIndex = parts.indexOf('membership');
        if (membershipIndex > 0) {
            return '/' + parts.slice(0, membershipIndex).join('/') + '/account/';
        }
        const docLang = String(document.documentElement.getAttribute('lang') || '').trim();
        return docLang ? '/' + docLang + '/account/' : '/account/';
    }
    ...
    function injectMembershipThemeFallback() {
        ...
        style.textContent = `
            ...
            .sk-membership-button:disabled,
            .sk-membership-button[aria-disabled="true"] {
                cursor: not-allowed;
            ...
            html.dark .sk-membership-button:disabled,
            body.dark .sk-membership-button:disabled,
            html[data-theme="dark"] .sk-membership-button:disabled,
            body[data-theme="dark"] .sk-membership-button:disabled,
            html.dark .sk-membership-button[aria-disabled="true"],
            body.dark .sk-membership-button[aria-disabled="true"],
            html[data-theme="dark"] .sk-membership-button[aria-disabled="true"],
            body[data-theme="dark"] .sk-membership-button[aria-disabled="true"] {
    ...
    function isMembershipActionLockedForRole(roleName) {
        const currentRole = normalizeRoleName(roleName);
        return currentRole === 'Lifetime Member' || currentRole === 'Administrator' || currentRole === 'Developer';
    }

    const STUDENT_STATUS_DEFAULT = {
        ok: 1,
        status: 'AVAILABLE',
        can_apply: 1,
    ...
    function normalizeStudentStatus(payload) {
        const source = payload && typeof payload === 'object' ? payload : {};
        return {
            ok: source.ok === 0 ? 0 : 1,
            status: String(source.status || (source.pending_role === 'Student Member' ? 'PENDING' : 'AVAILABLE')),
            can_apply: Number(source.can_apply == null ? 1 : source.can_apply) === 1 ? 1 : 0,
            current_role: String(source.current_role || 'Non-member'),
            pending_role: source.pending_role || null,
        };
    }

    async function fetchStudentMembershipStatus() {
        try {
            const data = await fetchJson(buildMembershipAjaxUrl('ajax_add_student_membership/'), { method: 'GET', credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
            return normalizeStudentStatus(data);
        } catch (error) {
            return Object.assign({}, STUDENT_STATUS_DEFAULT, { status: 'AVAILABLE', can_apply: 1 });
        }
    }

    function getStudentUiState(lang, studentStatus, isLoading) {
        const status = normalizeStudentStatus(studentStatus);
        if (isLoading) {
            return {
                noteText: t(lang, 'form.note_student_loading'),
                buttonLabel: t(lang, 'form.button_processing'),
                disabled: true,
            };
        }
        if (status.status === 'PENDING' || status.status === 'ALREADY_PENDING') {
            return {
                noteText: t(lang, 'form.note_student_pending'),
                buttonLabel: t(lang, 'form.button_pending'),
                disabled: true,
            };
        }
        if (status.status === 'ALREADY_ACTIVE') {
            return {
                noteText: t(lang, 'form.note_student_active'),
                buttonLabel: t(lang, 'form.button_active'),
                disabled: true,
            };
        }
        if (status.status === 'NOT_ELIGIBLE') {
            return {
                noteText: t(lang, 'form.note_student_ineligible'),
                buttonLabel: t(lang, 'form.button_register'),
                disabled: true,
            };
        }
        return {
            noteText: t(lang, 'form.note_student'),
            buttonLabel: t(lang, 'form.button_register'),
            disabled: false,
        };
    }
    ...
    function render(target, state) {
        ...
        const membershipActionLocked = isMembershipActionLockedForRole(state.studentStatus && state.studentStatus.current_role);
        const studentUi = selected.is_free
            ? getStudentUiState(lang, state.studentStatus, !!state.studentActionLoading)
            : null;
        const buttonLabel = selected.is_free
            ? studentUi.buttonLabel
            : t(lang, "form.button_checkout");
        const noteText = selected.is_free
            ? studentUi.noteText
            : t(lang, "form.note_paid");
        const isActionDisabled = selected.is_free
            ? !!(studentUi && studentUi.disabled)
            : membershipActionLocked;
        const buttonDisabledAttr = isActionDisabled ? ' disabled aria-disabled="true"' : '';
        const buttonDisabledClass = isActionDisabled ? ' opacity-70 cursor-not-allowed hover:translate-y-0' : '';
            ...
            <div class="sk-membership-root py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6" dir="${rtl ? "rtl" : "ltr"}">
                ...
                <div class="sk-membership-card mx-auto max-w-2xl rounded-3xl p-6 md:p-8">
                    ...
                    <div class="mt-8">
                        ...
                        <button type="button" id="membership_checkout_btn" class="sk-membership-button ${buttonTypeClass}${buttonDisabledClass}"${buttonDisabledAttr}>
        ...
        if (button) {
            button.addEventListener("click", async () => {
                ...
                if (currentSelected.is_free) {
                    const uiState = getStudentUiState(state.lang, state.studentStatus, !!state.studentActionLoading);
                    if (uiState.disabled || state.studentActionLoading) {
                        return;
                    }
                    state.studentActionLoading = true;
                    render(target, state);
                    try {
                        const data = await fetchJson(buildMembershipAjaxUrl('ajax_add_student_membership/'), {
                            method: 'POST',
                            credentials: 'same-origin',
                            headers: { 'X-CSRFToken': getCookie('csrftoken'), 'X-Requested-With': 'XMLHttpRequest' },
                        });
                        state.studentStatus = normalizeStudentStatus(data);
                        if (state.studentStatus.status === 'PENDING' || state.studentStatus.status === 'ALREADY_PENDING') {
                            alert(t(state.lang, 'alert.student_success'));
                        } else if (state.studentStatus.status === 'ALREADY_ACTIVE') {
                            alert(t(state.lang, 'alert.student_already_active'));
                        } else if (state.studentStatus.status === 'NOT_ELIGIBLE') {
                            alert(t(state.lang, 'alert.student_not_eligible'));
                        } else {
                            alert(t(state.lang, 'alert.student_request_failed'));
                        }
                    } catch (error) {
                        alert(t(state.lang, 'alert.student_request_failed'));
                    } finally {
                        state.studentActionLoading = false;
                        render(target, state);
                    }
                    return;
                }

                if (isMembershipActionLockedForRole(state.studentStatus && state.studentStatus.current_role)) {
                    return;
                }
    ...
    if (typeof gv_username === "undefined" || gv_username == null || gv_username === "") {
        window.location.href = buildAccountPath();
        return;
    }
    ...
    const state = {
        lang: getInitialLang(),
        selectedKey: "regular",
        studentStatus: Object.assign({}, STUDENT_STATUS_DEFAULT),
        studentActionLoading: false,
    };

    render(target, state);
    fetchStudentMembershipStatus().then((status) => {
        state.studentStatus = normalizeStudentStatus(status);
        if (state.studentStatus.status === 'PENDING' || state.studentStatus.status === 'ALREADY_ACTIVE') {
            state.selectedKey = 'student';
        }
        render(target, state);
    });
