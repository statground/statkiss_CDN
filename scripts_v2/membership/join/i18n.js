...
(function () {
    const api = window.StatKISS_MembershipI18N
    if (!api || !api.dict) return
    const extra = {
        en: {
            'form.note_student_pending': 'Your Student Member application is pending admin approval.',
            'form.note_student_active': 'You are already an active Student Member.',
            'form.note_student_ineligible': 'Only Non-member accounts can apply for Student Member.',
            'form.note_student_loading': 'Checking your Student Member status…',
            'form.button_pending': 'Pending Approval',
            'form.button_active': 'Already Registered',
            'form.button_processing': 'Processing…',
            'alert.student_success': 'Your Student Member application has been submitted. Please wait for admin approval.',
            'alert.student_already_pending': 'Your Student Member application is already pending admin approval.',
            'alert.student_already_active': 'You are already an active Student Member.',
            'alert.student_not_eligible': 'Only Non-member accounts can apply for Student Member.',
            'alert.student_request_failed': 'Unable to process the Student Member request.',
        },
        ko: {
            'form.note_student_pending': 'Student Member 신청이 접수되었으며 관리자 승인을 기다리고 있습니다.',
            'form.note_student_active': '이미 학생회원으로 활성화되어 있습니다.',
            'form.note_student_ineligible': '비회원만 Student Member를 신청할 수 있습니다.',
            'form.note_student_loading': 'Student Member 신청 상태를 확인하는 중입니다…',
            'form.button_pending': '승인 대기중',
            'form.button_active': '이미 등록됨',
            'form.button_processing': '처리 중…',
            'alert.student_success': 'Student Member 신청이 접수되었습니다. 관리자 승인을 기다려주세요.',
            'alert.student_already_pending': '이미 Student Member 승인 대기중입니다.',
            'alert.student_already_active': '이미 학생회원입니다.',
            'alert.student_not_eligible': '비회원만 Student Member를 신청할 수 있습니다.',
            'alert.student_request_failed': 'Student Member 신청을 처리하지 못했습니다.',
        },
    }
    const keys = Object.keys(extra.en)
    api.UI_KEYS = Array.from(new Set([].concat(api.UI_KEYS || [], keys)))
    const languageCodes = (api.languages || []).map(item => item.code)
    languageCodes.forEach(code => {
        if (!api.dict[code]) api.dict[code] = {}
        keys.forEach(key => {
            if (api.dict[code][key] == null || api.dict[code][key] === '') {
                if (code === 'ko' && extra.ko[key]) api.dict[code][key] = extra.ko[key]
                else api.dict[code][key] = extra.en[key]
            }
        })
    })
    if (typeof api.normalizeCoverage === 'function') api.normalizeCoverage()
})();
