...
function getLangFromPath(){
    try {
        ...
        const initial = resolveLangCode(parts[0]);
...
function getLang(){
    ...
    const initial = resolveLangCode(htmlLang || saved || browser);
    const best = languages.find(l => l.code === initial)
        || languages.find(l => initial.startsWith(l.code))
        || languages.find(l => l.code.startsWith(initial.split('-')[0]))
        || languages[0];
...
function parse_admin_shell_rgb(value) {
...
function get_admin_shell_mode_from_background(value) {
    const rgb = parse_admin_shell_rgb(value);
    if (!rgb) {
        return null;
    }
...
function detect_admin_shell_theme_mode() {
    try {
    ...
    for (const target of targets) {
        ...
        if (/\blight\b|\btheme-light\b|\blight-mode\b/i.test(className)) {
            return 'light';
        }
    }

    for (const target of targets) {
        const mode = get_admin_shell_mode_from_background(window.getComputedStyle(target).backgroundColor);
        if (mode) {
            return mode;
        }
    }
...
function get_admin_menu_palette(themeMode) {
    if (themeMode === 'dark') {
        return {
            activeBg: '#2563eb',
            activeText: '#eff6ff',
            activeBorder: '#60a5fa',
            activeShadow: '0 12px 28px rgba(37, 99, 235, 0.32)',
            inactiveBg: '#0f172a',
            inactiveText: '#e2e8f0',
            inactiveBorder: '#334155',
            inactiveShadow: '0 10px 24px rgba(2, 6, 23, 0.28)',
    ...
    return {
        ...
        activeText: '#1d4ed8',
        activeBorder: '#93c5fd',
        activeShadow: '0 10px 24px rgba(59, 130, 246, 0.18)',
        inactiveBg: '#ffffff',
        inactiveText: '#0f172a',
        inactiveBorder: '#cbd5e1',
        inactiveShadow: '0 8px 18px rgba(15, 23, 42, 0.08)',
...
function Div_admin_menu(props) {
    ...
    const [themeMode, setThemeMode] = React.useState(detect_admin_shell_theme_mode());
    const items = [
    ...
    const palette = get_admin_menu_palette(themeMode);

    React.useEffect(() => {
        const refreshThemeMode = () => {
            const nextMode = detect_admin_shell_theme_mode();
    ...
    function Div_menu_button(buttonProps) {
        ...
        const buttonClass = 'relative inline-flex items-center justify-center mb-2 me-2 rounded-lg border px-5 py-2.5 text-sm font-semibold whitespace-nowrap transition duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-4 ' + palette.focusRing;
        const buttonStyle = isActive
            ...
            : {
                ...
                boxShadow: palette.inactiveShadow
            };
    ...
    return (
        <div class="flex flex-wrap justify-center items-center w-full gap-y-2">
