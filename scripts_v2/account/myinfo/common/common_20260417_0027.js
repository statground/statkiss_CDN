(function () {
    ...
    const THEME_ATTRS = ['data-theme', 'theme', 'data-color-mode', 'data-bs-theme', 'data-mode', 'data-statkiss-theme', 'color-scheme'];
    const localMessages = {};
    ...
    function currentLocalePrefix() {
        const pathSegments = splitPath(window.location.pathname || '');
        ...
        const candidates = [
            getBootstrap().lang,
            getApp().lang,
            safeLocalStorageGet(LOCAL_LANG_KEY),
        ...
        for (let index = 0; index < candidates.length; index += 1) {
            ...
            if (SUPPORTED_LANGS.indexOf(normalized) >= 0) {
                return '/' + normalized;
            }
        }
        return '';
    ...
    function withLocalePrefix(path) {
        ...
        const firstNormalized = normalizeLangCode(segments[0]);
        if (SUPPORTED_LANGS.indexOf(firstNormalized) >= 0 && segments[0].toLowerCase() !== 'account') {
            segments[0] = prefix.slice(1);
...
function normalizeAbsoluteRoutePath(path, fallbackPath) {
    ...
    if (raw.charAt(0) === '/') {
        return withLocalePrefix(raw);
    }
    const cleaned = raw.replace(/^\.\//, '').replace(/^\//, '');
    if (cleaned.indexOf('account/') === 0) {
        return withLocalePrefix('/' + cleaned);
    }
    if (cleaned.indexOf('google/') === 0) {
        return withLocalePrefix('/account/' + cleaned);
    }
    if (cleaned.indexOf('myinfo/connections/google/link') === 0) {
        return withLocalePrefix('/account/google/link/');
    }
    return withLocalePrefix(fallbackPath);
}
    ...
    function route(key, fallbackPath) {
        ...
        return normalizeAbsoluteRoutePath(routes[key], fallbackPath);
    }

    function api(key, fallbackPath) {
        const endpoints = getBootstrap().api || {};
        return normalizeAbsoluteRoutePath(endpoints[key], fallbackPath);
    }
    ...
    function myinfoRoute(mode) {
        ...
        if (mode === 'connections') {
            return route('myinfoConnections', '/account/myinfo/connections/');
        }
        return route('myinfoOverview', route('myinfo', '/account/myinfo/'));
    }
    ...
    function ensureThemeStyles() {
        ...
        style.textContent = [
            ...
            '.sk-myinfo-root .sk-myinfo-tone-box.is-info{background:#eff6ff;border-color:#bfdbfe;color:#1d4ed8;}',
            '.sk-myinfo-root .sk-myinfo-tone-box.is-success{background:#ecfdf5;border-color:#a7f3d0;color:#047857;}',
            '.sk-myinfo-root .sk-myinfo-tone-box.is-error{background:#fef2f2;border-color:#fecaca;color:#b91c1c;}',
            '.sk-myinfo-root.sk-theme-dark .sk-myinfo-tone-box.is-info{background:rgba(8,31,63,.92) !important;border-color:rgba(96,165,250,.34) !important;color:#cbe7ff !important;}',
            '.sk-myinfo-root.sk-theme-dark .sk-myinfo-tone-box.is-success{background:rgba(5,46,35,.92) !important;border-color:rgba(16,185,129,.36) !important;color:#b7f7dd !important;}',
            '.sk-myinfo-root.sk-theme-dark .sk-myinfo-tone-box.is-error{background:rgba(69,10,10,.90) !important;border-color:rgba(248,113,113,.36) !important;color:#fecaca !important;}'
        ].join('');
        ...
        if (typeof MutationObserver === 'function') {
            ...
            const observeTarget = function (target) {
                ...
                observer.observe(target, {
                    attributes: true,
                    attributeFilter: ['class', 'style', 'data-theme', 'theme', 'data-color-mode', 'data-bs-theme', 'data-mode', 'data-statkiss-theme', 'color-scheme'],
                });
            };
...
function noticeBox(message, tone) {
    const toneClass = {
        info: 'sk-myinfo-tone-box is-info rounded-xl border px-4 py-4 text-sm',
        success: 'sk-myinfo-tone-box is-success rounded-xl border px-4 py-4 text-sm',
        error: 'sk-myinfo-tone-box is-error rounded-xl border px-4 py-4 text-sm'
    }[tone || 'info'] || 'sk-myinfo-tone-box is-info rounded-xl border px-4 py-4 text-sm';
    ...
    function submenu(currentMode) {
        const items = [
            ...
            { mode: 'connections', label: t('myinfo_nav_connections'), href: myinfoRoute('connections') },
        ];
