window.set_main = function () {
    ...
    function localizeDynamicOptions() {
        ...
        normalizeGenderSelectOptions();
        ui.localizeSelectOptions(document.getElementById('sel_education'), 'education');
    }

    function translatedText(keys, fallback) {
        if (!Array.isArray(keys)) {
            return fallback;
        }
        for (let i = 0; i < keys.length; i += 1) {
            const key = keys[i];
            if (!key || !ui || typeof ui.t !== 'function') {
                continue;
            }
            const value = String(ui.t(key) || '').trim();
            if (value && value !== key) {
                return value;
            }
        }
    ...
    function preferNotToSayLabel() {
        return translatedText([
            'gender_prefer_not_to_say',
            'gender_prefer_not_say',
            'prefer_not_to_say',
            'myinfo_gender_prefer_not_to_say'
        ], 'Prefer not to say');
    }

    function otherOptionLabels() {
        const labels = ['other'];
        [
            'gender_other',
            'other',
            ...
            if (value) {
                labels.push(String(value).trim().toLowerCase());
            }
        });
        return new Set(labels);
    }

    function normalizeGenderSelectOptions() {
        const genderSelect = document.getElementById('sel_gender');
        if (!genderSelect || !genderSelect.options || !genderSelect.options.length) {
            return;
        }

        const options = Array.from(genderSelect.options);
        const duplicateOtherOptions = [];
        const otherLabels = otherOptionLabels();
        let preferNotToSayOption = null;
        ...
        options.forEach(function (option) {
            ...
            const visibleText = String(option.textContent || option.label || '').trim().toLowerCase();
            const metaText = [
                option.value,
                option.getAttribute ? option.getAttribute('data-name') : '',
                option.getAttribute ? option.getAttribute('data-label') : '',
                option.getAttribute ? option.getAttribute('data-key') : '',
                option.getAttribute ? option.getAttribute('data-code') : ''
            ].join(' ').toLowerCase();

            if (otherLabels.has(visibleText)) {
                duplicateOtherOptions.push(option);
            }

            if (
                metaText.indexOf('prefer not to say') !== -1 ||
                metaText.indexOf('prefer_not_to_say') !== -1 ||
                metaText.indexOf('prefer-not-to-say') !== -1
            ) {
                preferNotToSayOption = option;
            }
        });

        const fixedLabel = preferNotToSayLabel();

        if (preferNotToSayOption) {
            preferNotToSayOption.textContent = fixedLabel;
            preferNotToSayOption.label = fixedLabel;
            return;
        }

        if (duplicateOtherOptions.length >= 2) {
            const lastOtherOption = duplicateOtherOptions[duplicateOtherOptions.length - 1];
            lastOtherOption.textContent = fixedLabel;
            lastOtherOption.label = fixedLabel;
        }
    }
