(function initStatkissAnnouncementI18N() {
  ...
  const TEXT = {
    en: {
      ...
      translatingBody: "Preparing this article in your selected language.",
    ...
    ko: {
      ...
      saving: "저장 중…",
      translating: "번역 중…",
      translatingBody: "선택한 언어로 글을 준비하고 있습니다.",
  ...
  const EXTRA_TEXT = {
    ja: { translating: "翻訳中…", translatingBody: "選択した言語で記事を準備しています。" },
    "zh-Hans": { translating: "翻译中…", translatingBody: "正在用所选语言准备这篇文章。" },
    "zh-Hant": { translating: "翻譯中…", translatingBody: "正在以所選語言準備這篇文章。" },
    es: { translating: "Traduciendo…", translatingBody: "Preparando este artículo en el idioma seleccionado." },
    fr: { translating: "Traduction…", translatingBody: "Préparation de cet article dans la langue sélectionnée." },
    de: { translating: "Wird übersetzt…", translatingBody: "Dieser Beitrag wird in Ihrer gewählten Sprache vorbereitet." },
  };
  ...
  function t(key, lang) {
    const normalized = normalizeLang(lang);
    const bundle = getTextBundle(normalized);
    if (bundle[key]) return bundle[key];
    if (EXTRA_TEXT[normalized] && EXTRA_TEXT[normalized][key]) return EXTRA_TEXT[normalized][key];
