  ...
  function ensureDonationStyles() {
    ...
    style.textContent = `
      ...
      .sk-donations-root.sk-mode-dark .sk-donor-icon-img {
        filter: brightness(0) invert(1) drop-shadow(0 0 1px rgba(255,255,255,0.85)) drop-shadow(0 0 6px rgba(255,255,255,0.18));
      }
    `;
    ...
    function DivDonorCard(props) {
      ...
      return (
        <div class="sk-card sk-donor-card rounded-2xl p-4 sm:p-5 text-center h-full flex flex-col justify-start items-center">
          ...
          {item.url_icon ? (
            <div class="mx-auto mb-4 w-12 h-12 rounded-lg sk-highlight-panel flex items-center justify-center">
              <img class="sk-donor-icon-img" src={item.url_icon} alt="" />
            </div>
          ) : (
