(function() {
  ...
  function renderContactCards() {
    if (state.loading) {
      return `<div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full animate-pulse">${Array.from({ length: 2 }).map(() => '<div class=\"sk-skeleton-block h-40 rounded-2xl\"></div>').join('')}</div>`;
    }
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
        <div class="space-y-2">
          <div class="flex flex-wrap items-center gap-3">
            <h3 class="sk-page-title text-xl font-bold">${escapeHtml(t('manage_years'))}</h3>
  ...
  function renderMain() {
    ...
    const donationSectionHtml = isEditMode() ? renderEditorSection() : renderPublicDonationSection();
