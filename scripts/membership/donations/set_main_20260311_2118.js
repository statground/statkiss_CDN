(function(){
  ...
  window.set_main = function set_main() {
    ...
    function DivSubHeader(props) {
      return (
        <h2 class="sk-section-title text-xl font-extrabold tracking-tight">
          {props.text}
        </h2>
      );
    }
    ...
    function DivSubDonationHeader(props) {
      return (
        <h3 class="sk-sub-title text-base font-semibold leading-7">
          {props.text}
        </h3>
      );
    }
    ...
    function DivBenefitTable() {
      ...
      return (
        <div class="flex flex-col justify-center items-start py-8 px-4 mx-auto max-w-screen-xl">
          ...
          <div class="w-full overflow-x-auto rounded-2xl sk-card">
    ...
    function DivAddDonorForm() {
      ...
      return (
        <div class="sk-card rounded-3xl p-6 sm:p-7">
          <div class="flex flex-col w-full justify-end items-center space-y-5">
            ...
            <div class="flex flex-col w-full space-y-4">
              <div class="grid grid-cols-3 w-full gap-6 md:grid-cols-1 md:gap-4">
            ...
            <div class="flex flex-row justify-end items-center w-full">
              <button type="button" onClick={handleSubmit} disabled={state.isSubmitting} class="sk-button-primary text-sm w-full md:w-auto text-center inline-flex items-center justify-center gap-2">
                {state.isSubmitting ? (
    ...
    function DivDonationSkeleton() {
      return (
        <div class="flex flex-col justify-center items-start py-8 px-4 mx-auto max-w-screen-xl space-y-8 animate-pulse">
    ...
    function DivYearBlock(props) {
      return (
        <div class="w-full rounded-3xl sk-soft-panel p-6 sm:p-7">
    ...
    function DivDonorCard(props) {
      ...
      return (
        <div class="sk-card rounded-2xl p-5 text-center h-full flex flex-col justify-start items-center">
          ...
          <h5 class="mb-1 text-lg font-bold tracking-tight sk-page-title">{item.name}</h5>
          <p class="sk-muted-text text-xs mb-1">{item.affiliation}</p>
          <p class="sk-body-text text-sm font-semibold mb-1">{formatMoney(item.amount, state.lang)}</p>
          {hasComment ? <p class="sk-muted-text text-xs mb-1">{item.comment}</p> : null}
        </div>
      );
    }
    ...
    function DivMain() {
      return (
        <div class={'sk-donations-root ' + (state.isDark ? 'sk-mode-dark' : 'sk-mode-light') + ' py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6'} dir={isRTL() ? 'rtl' : 'ltr'}>
          <div class="mx-auto max-w-screen-lg text-center">
            <h2 class="sk-page-title mb-2 text-4xl tracking-tight font-extrabold">
              {t('title')}
            </h2>
            <p class="sk-body-text mb-8 lg:text-lg leading-8">
              {t('subtitle')}
            </p>
          </div>
          ...
          <div class="flex flex-col justify-center items-start py-8 px-4 mx-auto max-w-screen-xl">
            ...
            <div class="w-full rounded-3xl sk-soft-panel px-6 py-6 sm:px-7">
          ...
          <div class="flex flex-col justify-center items-start py-8 px-4 mx-auto max-w-screen-xl">
            ...
            <div class="grid grid-cols-2 items-start gap-6 w-full md:grid-cols-1">
              <div class="sk-card rounded-3xl p-6 sm:p-7 flex flex-col w-full px-6 space-y-4">
              ...
              <div class="sk-card rounded-3xl p-6 sm:p-7 flex flex-col w-full px-6">
          ...
          {/* TODO(statkiss): Re-enable the Whom to contact block after the fetch contract is redesigned.
              fetch('/membership/ajax_get_whom_to_contact/') is intentionally disabled for now. */}
    ...
    async function handleSubmit() {
      ...
      try {
        ...
        const response = await fetch('/membership/ajax_add_donator/', {
          method: 'post',
    ...
    async function handleDelete(uuid) {
      ...
      try {
        ...
        const response = await fetch('/membership/ajax_delete_donator/', {
          method: 'post',
          headers: { 'X-CSRFToken': getCsrfToken() },
          body: inputData,
        });
    ...
    async function loadDonations() {
      try {
        const response = await fetch('/membership/ajax_get_donation_list/');
        if (!response.ok) {
