function set_main() {
    ...
    async function fetchMembershipJson(endpoint, options) {
        const response = await fetch(buildMembershipAjaxUrl(endpoint), Object.assign({ credentials: 'same-origin', headers: { 'Accept': 'application/json' } }, options || {}));
    ...
    async function fetchStudentMembershipStatus() {
        try {
            const data = await fetchMembershipJson('ajax_add_student_membership/', { method: 'GET' });
        ...
        if (button) {
            button.addEventListener("click", async () => {
                ...
                if (currentSelected.is_free) {
                    ...
                    render(target, state);
                    try {
                        const data = await fetchMembershipJson('ajax_add_student_membership/', {
                            method: 'POST',
