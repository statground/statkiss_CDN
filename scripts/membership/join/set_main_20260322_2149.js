function set_main() {
    ...
    function pickSupportedLang(code) {
        ...
        const exact = SUPPORTED_LANGUAGES.find((item) => item.code === resolved);
        if (exact) return exact.code;

        const startsWithResolved = SUPPORTED_LANGUAGES.find((item) => item.code.indexOf(resolved) === 0);
        if (startsWithResolved) return startsWithResolved.code;

        const sameBase = SUPPORTED_LANGUAGES.find((item) => item.code.split("-")[0] === shortCode);
        if (sameBase) return sameBase.code;

        return "en";
    }

    function normalizeMenuCoverage() {
        const en = MENU_DICT.en || {};
        const ko = MENU_DICT.ko || {};
        ...
        SUPPORTED_LANGUAGES.forEach((item) => {
            ...
            MENU_UI_KEYS.forEach((key) => {
                if (MENU_DICT[code][key] == null || MENU_DICT[code][key] === "") {
                    MENU_DICT[code][key] = en[key] != null && en[key] !== ""
                        ? en[key]
    ...
    function menuT(lang, key) {
        const code = pickSupportedLang(lang);
        const current = MENU_DICT[code] || {};
        const en = MENU_DICT.en || {};
        const ko = MENU_DICT.ko || {};
        ...
        if (HEADER_I18N && typeof HEADER_I18N.t === "function") {
            return HEADER_I18N.t(code, key);
        }
    ...
    function isRTL(lang) {
        if (HEADER_I18N && typeof HEADER_I18N.isRTL === "function") {
            return HEADER_I18N.isRTL(lang);
        }
        return lang === "ar";
    }
    ...
    function applyLangToDocument(lang) {
        ...
        if (HEADER_I18N && typeof HEADER_I18N.applyLangToDocument === "function") {
            HEADER_I18N.applyLangToDocument(code);
            return;
        }
    ...
    function useMenuLang() {
        ...
        React.useEffect(() => {
            ...
            document.addEventListener("visibilitychange", syncLang);
            ...
            const timerId = window.setInterval(syncLang, 800);

            return () => {
                window.removeEventListener("storage", syncLang);
                window.removeEventListener("languagechange", syncLang);
                window.removeEventListener("focus", syncLang);
                document.removeEventListener("visibilitychange", syncLang);
    ...
    function getMemberships(lang) {
        return [
            {
                key: "regular",
                title: menuT(lang, "membership.regular.title"),
                price_label: "$30",
                price_suffix: menuT(lang, "membership.regular.price_suffix"),
                description: menuT(lang, "membership.regular.description"),
                ncp_link_id: "KPGQBVGSXDRZN",
                is_free: false,
            },
            {
                key: "lifetime",
                title: menuT(lang, "membership.lifetime.title"),
                price_label: "$500",
                price_suffix: menuT(lang, "membership.lifetime.price_suffix"),
                description: menuT(lang, "membership.lifetime.description"),
                ncp_link_id: "FKR34ARDG8VHC",
                is_free: false,
            },
            {
                key: "spouse",
                title: menuT(lang, "membership.spouse.title"),
                price_label: "$10",
                price_suffix: menuT(lang, "membership.spouse.price_suffix"),
                description: menuT(lang, "membership.spouse.description"),
                ncp_link_id: "QVTLS9Q4U6YKL",
                is_free: false,
            },
            {
                key: "joint",
                title: menuT(lang, "membership.joint.title"),
                price_label: "$10",
                price_suffix: menuT(lang, "membership.joint.price_suffix"),
                description: menuT(lang, "membership.joint.description"),
                ncp_link_id: "YWUXLGWN6C6DS",
                is_free: false,
            },
            {
                key: "student",
                title: menuT(lang, "membership.student.title"),
                price_label: menuT(lang, "membership.free"),
                price_suffix: menuT(lang, "membership.student.price_suffix"),
                description: menuT(lang, "membership.student.description"),
                ncp_link_id: null,
                is_free: true,
            },
        ];
    ...
    const Div_main = () => {
        ...
        const memberships = React.useMemo(() => getMemberships(lang), [lang]);
        const [selectedKey, setSelectedKey] = React.useState("regular");
        const selected = memberships.find((item) => item.key === selectedKey) || memberships[0];
        ...
        const onClickCheckout = () => {
            if (selected.key === "student") {
                if (typeof click_btn_register_student === "function") {
                    ...
                    alert(menuT(lang, "alert.student_handler_missing"));
                }
                return;
            }

            if (!selected.ncp_link_id) {
                alert(menuT(lang, "alert.paypal_id_missing"));
                return;
            }
        ...
        const class_outer = "py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6 text-slate-900 dark:text-slate-100 transition-colors";
        const class_header = "mx-auto max-w-screen-md text-center mb-8 lg:mb-12";
        const class_card = "mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white/95 p-6 md:p-8 shadow-xl shadow-slate-200/40 transition-colors backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/95 dark:shadow-black/30";
        const class_title = "mb-2 text-center text-4xl font-extrabold tracking-tight text-slate-900 transition-colors dark:text-white";
        const class_desc = "mb-8 text-center text-slate-600 transition-colors dark:text-slate-300";
        const class_price_wrap = "my-6 flex items-baseline justify-center gap-2";
        const class_price = "text-6xl font-extrabold text-blue-900 transition-colors dark:text-sky-300";
        const class_price_suffix = "text-xl text-slate-500 transition-colors dark:text-slate-400";
        const class_label = `block mb-2 text-sm font-semibold text-slate-700 transition-colors dark:text-slate-200 ${rtl ? "text-right" : "text-left"}`;
        const class_select = "block w-full rounded-lg border border-slate-300 bg-white p-3 text-base text-slate-900 transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-sky-400 dark:focus:ring-sky-900/60";
        const class_note = `mt-3 text-sm text-slate-500 transition-colors dark:text-slate-400 ${rtl ? "text-right" : "text-left"}`;
        const class_btn = "mt-6 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-5 py-3 text-base font-bold text-white transition-colors hover:bg-slate-800 focus:ring-4 focus:ring-slate-200 dark:bg-sky-500 dark:text-slate-950 dark:hover:bg-sky-400 dark:focus:ring-sky-900/60";
        const class_btn_free = "mt-6 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-amber-400 px-5 py-3 text-base font-bold text-slate-900 transition-colors hover:bg-amber-300 focus:ring-4 focus:ring-amber-200 dark:bg-amber-300 dark:text-slate-950 dark:hover:bg-amber-200 dark:focus:ring-amber-900/50";
        const class_selected_title = "text-2xl font-extrabold text-slate-900 transition-colors dark:text-white";
        const class_selected_desc = "text-slate-600 transition-colors dark:text-slate-300";
        const class_footer = "mt-3 text-center text-xs text-slate-400 transition-colors dark:text-slate-500";

        return (
            <div className={class_outer} dir={rtl ? "rtl" : "ltr"}>
                <div className={class_header}>
                    <h2 className={class_title}>{menuT(lang, "page.join")}</h2>
                    <p className={class_desc}>{menuT(lang, "page.subtitle")}</p>
                </div>

                <div className={class_card}>
                    <div className="text-center">
                        <h3 className={class_selected_title}>{selected.title}</h3>

                        <div className={class_price_wrap}>
                            <span className={class_price}>{selected.price_label}</span>
                            {selected.price_suffix ? <span className={class_price_suffix}>{selected.price_suffix}</span> : null}
                        </div>

                        <p className={class_selected_desc}>{selected.description}</p>
                    </div>

                    <div className="mt-8">
                        <label className={class_label}>{menuT(lang, "form.membership_type")}</label>

                        <select className={class_select} value={selectedKey} onChange={onChangeSelect}>
                            {memberships.map((item) => (
                                <option key={item.key} value={item.key}>
                                    {item.is_free
                                        ? `${item.title} - ${menuT(lang, "membership.free")}`
                        ...
                        <p className={class_note}>
                            {selected.is_free
                                ? menuT(lang, "form.note_student")
                                : menuT(lang, "form.note_paid")}
                        </p>

                        {selected.is_free ? (
                            <button type="button" className={class_btn_free} onClick={onClickCheckout}>
                                {menuT(lang, "form.button_register")}
                            </button>
                        ) : (
                            <button type="button" className={class_btn} onClick={onClickCheckout}>
                                {menuT(lang, "form.button_checkout")}
                            </button>
                        )}

                        <div className={class_footer}>
                            {menuT(lang, "form.powered_by_paypal")}
                        </div>
                    </div>
                </div>
            </div>
