...
function set_main() {
  ...
  function buildSolidEditSrcDoc() {
    const scriptSrc = escapeHtmlAttribute(getSolidEditScriptSrc());
    const initialLang = escapeHtmlAttribute(getCurrentLang());
    return `<!DOCTYPE html>
<html lang="${initialLang}">
  <head>
    ...
    <style>
      ...
      .lre-root { padding: 0 !important; }
      .lre-app,
      .lre-surface,
      .lre-main-wrap,
      .lre-editor-shell,
      .lre-editor-shell.is-split {
      ...
      .lre-surface { padding: 0 !important; }
      .lre-editor-page {
        width: 100% !important;
      ...
      .lre-editor-page:focus { outline: none !important; }
      body.lre-dark { background: transparent !important; color: #ffffff !important; }
      body.lre-dark .lre-editor-page,
      body.lre-dark .lre-editor-page * {
        color: #ffffff;
      }
      body.lre-dark .lre-editor-page a { color: #93c5fd; }
      body.lre-dark .lre-editor-page table,
      body.lre-dark .lre-editor-page th,
      body.lre-dark .lre-editor-page td,
      body.lre-dark .lre-editor-page pre,
      body.lre-dark .lre-editor-page code,
      body.lre-dark .lre-editor-page blockquote {
        border-color: #334155 !important;
        background: rgba(15, 23, 42, 0.72) !important;
      }
    </style>
  ...
  function getSolidEditorApi(frame) {
    try {
      const win = frame && frame.contentWindow;
      if (!win) return null;
      return win.__localRichEditor || win.localRichEditor || null;
    } catch (_) {
      return null;
    }
  }

  function syncSolidEditorEnvironment(frame, options) {
    const api = getSolidEditorApi(frame);
    ...
    try {
      ...
      if (doc && doc.body) {
        doc.body.classList.toggle("lre-dark", dark);
      }
    } catch (_) {}
  ...
  function setSolidEditorHtml(frame, html) {
    const api = getSolidEditorApi(frame);
  ...
  function readSolidEditorHtml(frame) {
    const api = getSolidEditorApi(frame);
  ...
  function EditorApp() {
    ...
    const solidEditSrcDoc = useMemo(() => buildSolidEditSrcDoc(), []);
    ...
    function syncEditorEnvironment() {
      syncSolidEditorEnvironment(editorFrameRef.current, {
    ...
    function writeEditorHtml(nextHtml) {
      ...
      const applied = setSolidEditorHtml(editorFrameRef.current, value);
    ...
    function readEditorValue() {
      const api = getSolidEditorApi(editorFrameRef.current);
      if (api && api.editor) {
        try {
          return String(api.editor.innerHTML || "");
        } catch (_) {}
    ...
    return (
      <div className="py-6 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6">
        <div className="sk-ann-surface space-y-6 rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm md:p-8">
          ...
          <div className="sk-ann-editor-shell overflow-hidden rounded-3xl border border-slate-200 bg-white">
            {editorFailed ? (
              ...
              <iframe
                ref={editorFrameRef}
                title="Announcement body editor"
                srcDoc={solidEditSrcDoc}
                className="sk-ann-editor-frame h-[640px] w-full"
              ></iframe>
            )}
