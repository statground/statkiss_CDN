async function click_submit() {
	...
	let txt_content = $('#div_summernote').summernote('code').trim()
	...
	if (!toggle_click_submit) {
		...
		// 게시글 등록
			...
			request_data.append('uuid_article', uuid);
			request_data.append('txt_title', txt_title);
			request_data.append('txt_content', txt_content);
			

			const data = await fetch("/announcement/ajax_update_article/", {
								method: "post", 
			...
			location.href="/announcement/" + url + "/read/" + uuid + "/"
		}
