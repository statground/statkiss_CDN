...
(function initStatkissAnnouncementShared() {
  ...
  function isProbablyEditorScriptPath(value) {
    return /\.js(?:[?#].*)?$/i.test(String(value || "").trim());
  }

  function resolveAnnouncementBasePath(rawBasePath) {
    const direct = String(rawBasePath || "").trim();
    if (direct && !isProbablyEditorScriptPath(direct)) {
      return direct;
    }
  ...
  function getContext() {
    ...
    const legacyBasePath = String(ctx.basePath || "").trim();
    const editorSrcFromContext = String(ctx.editorSrc || ctx.solidEditorSrc || "").trim();
    const announcementBasePath = resolveAnnouncementBasePath(ctx.announcementBasePath || legacyBasePath);
    const editorSrc = editorSrcFromContext || (isProbablyEditorScriptPath(legacyBasePath) ? legacyBasePath : "");
  ...
  function getAnnouncementBasePath() {
    ...
    let base = ctx.announcementBasePath || ctx.basePath || "/announcement/";
...
function set_main() {
  ...
  function Spinner({ size = "h-8 w-8", tone = "text-blue-600" }) {
    return (
      <svg className={`sk-ann-spinner ${size} ${tone}`} viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="3" opacity="0.25"></circle>
  ...
  function BusyCard({ lang, mode = "loading" }) {
    ...
    return (
      <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6">
        <div className="sk-ann-surface rounded-3xl border border-slate-200 bg-slate-50 px-6 py-12 text-center shadow-sm">
    ...
    return (
      <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6">
        <div className="rounded-3xl border border-red-200 bg-red-50 px-6 py-8 text-center shadow-sm">
          <div className="text-base font-semibold text-red-600">{message || t("failedLoad", lang)}</div>
        </div>
      </div>
    );
  }
  ...
  function AttachmentList({ items, lang }) {
    ...
    return (
      <section className="mt-8 space-y-3">
        <div className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{t("attachments", lang)}</div>
        <div className="flex flex-col gap-2">
          {items.map((item) => (
            <a
              ...
              className="sk-ann-card flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 hover:bg-slate-50"
            >
              <span className="truncate text-sm font-medium text-slate-900">{item.file_name}</span>
              <span className="shrink-0 text-xs text-slate-500">{formatFileSize(item.file_size)}</span>
            </a>
          ))}
        </div>
      </section>
  ...
  function AttachmentDropzone({ lang, existingItems, setExistingItems, newFiles, setNewFiles }) {
    ...
    return (
      <section className="space-y-4">
        <div>
          <div className="mb-2 text-sm font-semibold text-slate-700">{t("uploadTitle", lang)}</div>
          <button
            type="button"
          ...
          >
            ...
            <div className="text-xs text-slate-500">{dragging ? t("dragActive", lang) : t("browse", lang)}</div>
          </button>
          <input ref={inputRef} type="file" multiple hidden onChange={(event) => acceptFiles(event.target.files)} />
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {existingItems.map((item) => (
                <div key={item.uuid} className="sk-ann-chip inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {newFiles.map((file) => {
                const key = `${file.name}::${file.size}::${file.lastModified}`;
                return (
                  <div key={key} className="sk-ann-chip inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700">
  ...
  function EmptyState({ lang }) {
    return (
      <div className="px-6 py-16 text-center text-slate-500">
        <div className="text-sm font-medium">{t("noPosts", lang)}</div>
      </div>
    );
  }
      ...
      <div className="divide-y divide-slate-100">
        {[0, 1, 2].map((key) => (
          <div key={key} className="px-4 py-4">
            <div className="animate-pulse rounded-2xl border border-slate-200 bg-white px-4 py-5">
  ...
  function ListApp() {
    ...
    if (error && !loading && rows.length === 0) return <ErrorCard lang={lang} message={error} />;
    ...
    return (
      <div className="py-6 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6 space-y-8">
        <div className="mx-auto max-w-screen-lg text-center space-y-3">
          <div className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{getCategoryTitle(ctx.url, lang)}</div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">{getCategoryTitle(ctx.url, lang)}</h1>
          {getCategoryDescription(ctx.url, lang) ? <p className="mx-auto max-w-3xl text-sm md:text-base text-slate-500">{getCategoryDescription(ctx.url, lang)}</p> : null}
        </div>

        <section className="space-y-4">
          <header className="flex items-center justify-between gap-3 border-b border-slate-200 pb-4">
            <div className="text-[11px] font-semibold uppercase tracking-[0.25em] text-slate-500">{getCategoryTitle(ctx.url, lang)}</div>
            {canManage ? (
              <button type="button" onClick={() => { window.location.href = buildPagePath("write"); }} className="inline-flex items-center justify-center rounded-full bg-blue-600 px-5 py-2.5 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-blue-700">
                {t("write", lang)}
              </button>
            ) : null}
          </header>
          ...
          <div className="sk-ann-surface overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
              ...
              <div className="divide-y divide-slate-100">
                {rows.map((row) => (
                  <div key={row.uuid} className="px-4 py-3">
                    <a href={buildPagePath("read", row.uuid)} className="block">
                      <article className="sk-ann-card group flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-4 py-4 transition-all duration-150 hover:border-slate-300 hover:bg-slate-50">
                          ...
                          <div className="flex-1 space-y-2">
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="sk-ann-chip inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1">
                                ...
                                <span className="text-[11px] font-semibold text-slate-700 tracking-wide">{getCategoryTitle(ctx.url, lang)}</span>
                              </div>
                              {row.is_new ? <span className="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-semibold tracking-wide text-amber-700">NEW</span> : null}
                            ...
                            <h2 className="text-base md:text-lg font-semibold text-slate-900 line-clamp-1">{row.title}</h2>
            ...
            {!loading && !hasMore && rows.length > 0 ? <div className="px-6 py-4 text-center text-xs text-slate-400">{t("allLoaded", lang)}</div> : null}
  ...
  function ReadApp() {
    ...
    useEffect(() => {
      ...
      const applyThemeAdaptation = () => {
        try { adaptRichContentForTheme(contentRef.current); } catch (_) {}
      };
    ...
    if (loading) return <BusyCard lang={requestedLang} mode={busyMode} />;
    if (error) return <ErrorCard lang={requestedLang} message={error} />;
    if (!displayedArticle) return <ErrorCard lang={requestedLang} message={t("failedLoad", requestedLang)} />;
      ...
      <div className="py-6 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6">
        <article className="sk-ann-surface overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {displayedArticle.is_new ? <span className="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-semibold text-amber-700">NEW</span> : null}
                  {canViewOriginal ? (
                    <button
                      type="button"
                      onClick={() => setViewMode(viewMode === "current" ? "original" : "current")}
                      className="sk-ann-original-btn inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold"
                    >
                ...
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900">{displayedArticle.title}</h1>
          ...
          <section className="bg-white px-6 py-8 md:px-8">
            <div ref={contentRef} className="sk-ann-content toastui-editor-contents max-w-none text-slate-800" dangerouslySetInnerHTML={{ __html: renderedContent }} />
            <AttachmentList items={attachments} lang={requestedLang} />
          </section>

          <footer className="border-t border-slate-200 bg-slate-50 px-6 py-4 md:px-8">
            <div className="flex flex-wrap justify-end gap-2">
              {canManage ? (
                <button type="button" onClick={() => { window.location.href = buildPagePath("write"); }} className="inline-flex items-center justify-center rounded-full bg-blue-600 px-4 py-2 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-blue-700">
                  {t("write", requestedLang)}
                </button>
              ) : null}
              {canManage ? (
                <button type="button" onClick={() => { window.location.href = buildPagePath("edit", ctx.uuid); }} className="inline-flex items-center justify-center rounded-full bg-slate-800 px-4 py-2 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-slate-900">
                  {t("edit", requestedLang)}
                </button>
              ) : null}
              {canManage ? (
                <button type="button" onClick={handleDelete} className="inline-flex items-center justify-center rounded-full bg-red-500 px-4 py-2 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-red-600">
                  {t("delete", requestedLang)}
                </button>
              ) : null}
              <button type="button" onClick={() => { window.location.href = buildPagePath(); }} className="sk-ann-button--ghost inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs md:text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
  ...
  function getSolidEditScriptSrc() {
    const fallback = "https://cdn.jsdelivr.net/gh/statground/solid-edit@main/latest/editor.js";
    const ctx = getContext();
    const next = String(window.STATKISS_SOLID_EDIT_SRC || ctx.editorSrc || fallback).trim();
    return next || fallback;
  }

  function injectSolidEditorEmbedStyles() {
    if (document.getElementById("statkiss-solid-editor-embed-style")) return;
    const style = document.createElement("style");
  ...
  function hasSolidEditorMountApi() {
    return typeof window.mountStatkissContentEditor === "function" || typeof window.mountEmbeddedContentEditor === "function";
  }

  function waitForSolidEditorApi(maxAttempts = 300, interval = 50) {
    return new Promise((resolve, reject) => {
      let attempts = 0;
      function poll() {
        if (hasSolidEditorMountApi()) {
          resolve(window.mountStatkissContentEditor || window.mountEmbeddedContentEditor);
          return;
        }
        attempts += 1;
        if (attempts > maxAttempts) {
          reject(new Error("Solid editor mount API was not found."));
          return;
        }
  ...
  function ensureSolidEditorScript() {
    if (hasSolidEditorMountApi()) return Promise.resolve(window.mountStatkissContentEditor || window.mountEmbeddedContentEditor);
    if (window.__STATKISS_SOLID_EDITOR_PROMISE) return window.__STATKISS_SOLID_EDITOR_PROMISE;

    const scriptSrc = getSolidEditScriptSrc();
    const normalizedSrc = new URL(scriptSrc, document.baseURI).href;
    injectSolidEditorEmbedStyles();
    window.LOCAL_RICH_EDITOR_AUTOSTART = false;
    window.STATKISS_SOLID_EDIT_AUTOINIT = false;

    window.__STATKISS_SOLID_EDITOR_PROMISE = new Promise((resolve, reject) => {
      let script = document.getElementById("statkiss-solid-editor-script");
      if (script && script.src && script.src !== normalizedSrc) {
        script.parentNode && script.parentNode.removeChild(script);
        script = null;
      }

      function handleError() {
        if (script) script.removeEventListener("error", handleError);
        window.__STATKISS_SOLID_EDITOR_PROMISE = null;
        reject(new Error(`Failed to load solid editor script: ${scriptSrc}`));
      }

      if (!script) {
        script = document.createElement("script");
        script.id = "statkiss-solid-editor-script";
        script.src = normalizedSrc;
        script.async = true;
        script.dataset.statkissSolidEditor = "true";
      ...
      waitForSolidEditorApi().then((api) => {
    ...
    return window.__STATKISS_SOLID_EDITOR_PROMISE;
  }
  ...
  function mountSolidEditorHost(host, options) {
    if (!host || !hasSolidEditorMountApi()) return null;
    const mountContentEditor = window.mountStatkissContentEditor || window.mountEmbeddedContentEditor;
    if (typeof mountContentEditor !== "function") return null;
    ...
    if (options && typeof options.html === "string") {
      textarea.value = options.html;
    }
    host.appendChild(textarea);

    return mountContentEditor(textarea, Object.assign({
      restoreDraft: true,
  ...
  function EditorApp() {
    ...
    function writeEditorHtml(nextHtml) {
      ...
      if (applied && editorInstanceRef.current && typeof editorInstanceRef.current.__hostMirrorNow === "function") {
        try { editorInstanceRef.current.__hostMirrorNow(true); } catch (_) {}
      }
    ...
    useEffect(() => {
      ...
      async function mountEditor() {
        ...
        try {
          await ensureSolidEditorScript();
          if (cancelled || mountTokenRef.current !== token) return;
          ...
          const instance = mountSolidEditorHost(host, {
            title: title || getCategoryTitle(ctx.url, lang),
    ...
    async function handleSubmit() {
      ...
      if (!editorReady && !editorFailed) return;
    ...
    return (
      <div className="py-6 px-4 mx-auto max-w-screen-xl sm:py-10 lg:px-6">
        <div className="sk-ann-surface space-y-6 rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm md:p-8">
          <div className="space-y-2">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{getCategoryTitle(ctx.url, lang)}</div>
            <input
              ref={titleInputRef}
              type="text"
              value={title}
          ...
          <div className="sk-ann-editor-shell rounded-3xl border border-slate-200 bg-white">
            {editorFailed ? (
              <div className="p-4 md:p-6">
                <textarea
                  ref={editorFallbackRef}
                  className="sk-ann-editor-fallback min-h-[560px] w-full rounded-2xl border border-slate-300 bg-white px-4 py-4 text-sm text-slate-900 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-300"
                  placeholder={t("contentRequired", lang)}
                  spellCheck="true"
                />
              </div>
            ) : (
              <div ref={editorHostRef} className="sk-ann-editor-host"></div>
            )}

            {loading || (!editorReady && !editorFailed) ? (
              <div className="sk-ann-editor-overlay absolute inset-0 flex items-center justify-center backdrop-blur-sm">
                <div className="flex items-center gap-3 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm">
                  <Spinner size="h-5 w-5" tone="text-blue-600" />
          ...
          <AttachmentDropzone lang={lang} existingItems={existingItems} setExistingItems={setExistingItems} newFiles={newFiles} setNewFiles={setNewFiles} />
          <div className="flex flex-wrap justify-end gap-2">
            <button type="button" onClick={handleSubmit} disabled={saving || loading || (!editorReady && !editorFailed)} className="inline-flex items-center justify-center rounded-full bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-70">
              {saving ? t("saving", lang) : t("submit", lang)}
            </button>
            <button type="button" onClick={() => { window.location.href = buildPagePath(); }} className="sk-ann-button--ghost inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
              {t("list", lang)}
            </button>
          </div>
        </div>
