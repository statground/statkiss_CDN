...
function set_main() {
  ...
  function hasSolidEditorMountApi() {
    return typeof window.initStatkissContentEditor === "function"
      || typeof getSolidEditorMountApi() === "function"
      || typeof window.autoMountStatkissContentEditor === "function";
  }

  function describeSolidEditorApiState() {
    return [
      `initStatkissContentEditor=${typeof window.initStatkissContentEditor}`,
      `mountStatkissContentEditor=${typeof window.mountStatkissContentEditor}`,
      `mountEmbeddedContentEditor=${typeof window.mountEmbeddedContentEditor}`,
      `autoMountStatkissContentEditor=${typeof window.autoMountStatkissContentEditor}`,
    ].join(", ");
  }
  ...
  function waitForSolidEditorApi(timeoutMs = 15000, interval = 50) {
    return new Promise((resolve, reject) => {
      ...
      function finish() {
        if (!hasSolidEditorMountApi()) return false;
      ...
      const timer = window.setInterval(() => {
        ...
        if (Date.now() - startedAt > timeout) {
          window.clearInterval(timer);
          reject(new Error(`Solid editor API was not found. Load editor.js from HTML before calling set_main(). (${describeSolidEditorApiState()})`));
        }
      }, interval);
    });
  }
    ...
    useEffect(() => {
      ...
      async function mountEditor() {
        ...
        try {
          ...
          try { console.error("[StatKISS] solid editor mount failed", error); } catch (_) {}
          if (cancelled || mountTokenRef.current !== token) return;
          destroySolidEditorInstance(editorInstanceRef.current);
          destroySolidEditorInstance(window.localRichEditor);
          editorInstanceRef.current = null;
