(function initStatkissAnnouncementRuntimeDefaults() {
    if (window.__STATKISS_ANNOUNCEMENT_RUNTIME_DEFAULTS__) return;
    window.__STATKISS_ANNOUNCEMENT_RUNTIME_DEFAULTS__ = true;
    window.LOCAL_RICH_EDITOR_AUTOSTART = false;
    window.STATKISS_SOLID_EDIT_AUTOINIT = false;
    window.STATKISS_SOLID_EDIT_CONFIG = Object.assign({
        placeholder: 'Write your content here.'
    }, window.STATKISS_SOLID_EDIT_CONFIG || {});
...
(function bootstrapStatkissAnnouncementPage() {
    if (window.__STATKISS_ANNOUNCEMENT_PAGE_BOOTSTRAPPED__) return;
    function run() {
        if (window.__STATKISS_ANNOUNCEMENT_PAGE_BOOTSTRAPPED__) return;
        if (!document.getElementById("div_main")) return;
        window.__STATKISS_ANNOUNCEMENT_PAGE_BOOTSTRAPPED__ = true;
        try {
            set_main();
        } catch (error) {
            window.__STATKISS_ANNOUNCEMENT_PAGE_BOOTSTRAPPED__ = false;
            try { console.error("[StatKISS] announcement bootstrap failed", error); } catch (_) {}
        }
    }
