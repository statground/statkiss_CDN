...
function set_main() {
    ...
    async function mountSolidEditorHost(host, options) {
        ...
        if (typeof window.createStatkissEditor === "function") {
            const booted = await window.createStatkissEditor(host, mergedOptions);
            if (booted)
                return booted;
        }
...
if (typeof window !== 'undefined') { window.set_main = set_main; }

if (typeof window !== 'undefined' && !window.__STATKISS_SET_MAIN_AUTO__) {
  window.__STATKISS_SET_MAIN_AUTO__ = true;
  const run = () => {
    try {
      ...
      console.error('[StatKISS] set_main auto run failed', err);
    }
  };
