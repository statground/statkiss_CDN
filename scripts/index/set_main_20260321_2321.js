...
function set_main() {
  ...
  const SUPPORTED_LANG_PREFIXES = ["ko", "en", "ja", "zh-Hans", "zh-Hant"];

  function getCurrentLangPrefix() {
    const pathname = (window.location && window.location.pathname) ? window.location.pathname : "/";
    const match = pathname.match(/^\/(ko|en|ja|zh-Hans|zh-Hant)(?=\/|$)/);
  ...
  function hasLangPrefix(pathname) {
    return SUPPORTED_LANG_PREFIXES.some((code) => pathname === `/${code}` || pathname.startsWith(`/${code}/`));
  }

  function localizeInternalHref(href) {
    if (!href) return "#";
    ...
    if (isExternalHref(raw)) {
      try {
        ...
        if (hasLangPrefix(parsed.pathname)) return parsed.pathname + parsed.search + parsed.hash;
        const prefix = getCurrentLangPrefix();
        return (prefix ? prefix + parsed.pathname : parsed.pathname) + parsed.search + parsed.hash;
      } catch (_) {
        return raw;
      }
    }
  ...
  function articleHref(item, lang) {
    if (!item) return "#";
    if (item.read_url) return localizeInternalHref(item.read_url);
    if (!item.url || !item.uuid) return "#";
    return localizeInternalHref(`/announcement/${item.url}/read/${item.uuid}/`);
  }
      ...
      <section className="flex flex-col gap-6">
        <div className="kiss-card rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/5 dark:bg-slate-900">
          ...
          <div className="grid grid-cols-1 gap-3">
            {items.map(it => (
              <a key={it.url} href={localizeInternalHref(it.url)}
        ...
        <div className="rounded-3xl bg-slate-900 p-8 shadow-2xl shadow-slate-900/20">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {loadingBanners ? (
              ...
              banners.map((b, idx) => (
                <a key={idx} href={isExternalHref(b.url) ? b.url : localizeInternalHref(b.url)} target="_blank" rel="noopener noreferrer"
