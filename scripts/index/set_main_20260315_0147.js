...
function set_main() {
  ...
  function articleHref(item, lang) {
    if (!item) return "#";
    if (item.read_url) return item.read_url;
    if (!item.url || !item.uuid) return "#";
    const safeLang = lang || getLang();
    return safeLang ? `/${safeLang}/announcement/${item.url}/read/${item.uuid}/` : `/announcement/${item.url}/read/${item.uuid}/`;
  }
