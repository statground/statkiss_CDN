...
function set_main() {
  ...
  async function fetchJson(url) {
    ...
    const now = Date.now();
    const ttlMs =
      url === "/ajax_get_article_list/" ? 60_000 :
        ...
        <div className="kiss-card rounded-3xl bg-white p-6 shadow-xl shadow-slate-900/5 dark:bg-slate-900">
          {isDesktop ? (
            <div className="grid" style={{ gridTemplateColumns:"1.8fr 1fr", gap:"2.5rem" }}>
              <div className="flex min-w-0 flex-col justify-between border-r border-slate-100 pr-10 dark:border-slate-800">
                <div>
                ...
                <div className="flex min-w-0 items-center justify-between gap-4 text-xs font-medium">
                  <span className="flex min-w-0 items-center gap-1.5 text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full dark:bg-slate-800 dark:text-slate-200">
              ...
              <div className="flex min-w-0 flex-col">
                ...
                <div className="flex min-w-0 flex-col divide-y divide-slate-50 dark:divide-slate-800">
      ...
      <section className="flex min-w-0 flex-col gap-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            {items.map(it => (
              <button key={it.label} type="button" onClick={() => it.url && (location.href = it.url)}
                ...
                <div className="flex min-w-0 h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-lg group-hover:bg-white dark:bg-slate-800 dark:group-hover:bg-slate-700">{it.icon}</div>
                <div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white p-6 text-slate-900 shadow-xl shadow-slate-900/5 dark:bg-gradient-to-br dark:from-indigo-600 dark:to-violet-600 dark:text-white dark:shadow-indigo-500/30">
          ...
          <div className="relative z-10 flex items-center gap-3 mb-4">
            <div className="flex min-w-0 h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-700 dark:bg-white/20 dark:text-white">
