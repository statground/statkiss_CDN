function input_checker() {
	...
	// 비밀번호 확인의 길이가 8자 미만으로 입력되었음
	} else if(document.getElementById("txt_password").value != document.getElementById("txt_password_confirm").value) {
		document.getElementById("desc_password_confirm_msg").className = class_desc_msg
	...
	// 이름이 입력되지 않음
	} else if(document.getElementById("txt_name").value.length < 0) {
	...
	// 소속이 입력되지 않음
	} else if(document.getElementById("txt_affiliation").value.length < 0) {
	...
	// 직위가 입력되지 않음
	} else if(document.getElementById("txt_title").value.length < 0) {
	...
	// 관심 분야가 입력되지 않음
	} else if(document.getElementById("txt_interest").value.length < 0) {
		...
		<Div_btn_submit id={"btn_submit"} class={class_btn_enabled} function={() => click_btn_submit()} text={"Sign Up"} />
