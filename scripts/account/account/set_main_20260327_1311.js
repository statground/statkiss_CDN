window.set_main = function () {
    ...
    const pageI18N = window.StatKISSAccountAccountI18N || window.StatKISSAccountLoginI18N || null;
    ...
    const pageMessages = window.StatKISSAccountAccountMessages || window.StatKISSAccountLoginMessages || {};
...
function googleLoginHref() {
    const route = app.routes && app.routes.googleLogin ? String(app.routes.googleLogin) : '';
    if (!route) return '';
    const next = getReturnToUrl();
    if (!next) return route;
    return route + (route.indexOf('?') >= 0 ? '&' : '?') + 'next=' + encodeURIComponent(next);
}
...
function renderGoogleLoginSection() {
    ...
    return [
        '<div class="space-y-3">',
        '  <a id="btn_google_login" href="' + escapeHtml(googleLoginHref()) + '" class="statkiss-account-outline-btn inline-flex h-12 w-full items-center justify-center rounded-2xl border border-slate-300 px-5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900">' + escapeHtml(t('continue_with_google')) + '</a>',
        '  <div class="relative py-1 text-center">',
        '      <div class="absolute inset-x-0 top-1/2 border-t border-slate-200 dark:border-slate-800"></div>',
        '      <span class="relative bg-white px-3 text-xs font-medium uppercase tracking-[0.12em] text-slate-400 dark:bg-slate-950 dark:text-slate-500">' + escapeHtml(t('or_continue_with_email')) + '</span>',
        '  </div>',
        '</div>'
    ].join('');
}

function showInitialPageMessage() {
    const message = GOOGLE_MESSAGE_MAP[app.pageMsg || ''];
    if (!message || typeof app.setNotice !== 'function') return;
    app.setNotice(t(message.key), message.kind || 'error');
}

    const bodyHtml = [
        renderGoogleLoginSection(),
    ...
    tagAuthScope('account_login_form', 'login');
