...
window.set_main = function () {
    ...
    const GOOGLE_MESSAGE_MAP = {
        ...
        GOOGLE_LINK_RESTRICTED: { key: 'google_link_restricted', kind: 'error' },
        PASSWORD_CHANGED: { key: 'change_password_success', kind: 'success' },
        BOT_VERIFICATION_REQUIRED: { key: 'bot_verification_required', kind: 'error' },
        BOT_VERIFICATION_RATE_LIMITED: { key: 'bot_verification_rate_limited', kind: 'error' },
        BOT_VERIFICATION_SEND_FAILED: { key: 'bot_verification_send_failed', kind: 'error' },
        BOT_VERIFIED: { key: 'bot_verified', kind: 'success' },
        BOT_VERIFY_INVALID: { key: 'bot_verify_invalid', kind: 'error' },
        BOT_VERIFY_EXPIRED: { key: 'bot_verify_expired', kind: 'error' }
    };
    ...
    function stickyNoticePayload(key, kind, params) {
        return {
            key: key,
            kind: kind || 'error',
            params: params || {},
    ...
    function renderStickyNotice(payload) {
        if (!payload || typeof app.setNotice !== 'function') return;
        app.setNotice(t(payload.key, payload.params || {}), payload.kind || 'error');
    }

    function setStickyNotice(key, kind, params) {
        stickyNotice = stickyNoticePayload(key, kind, params);
        renderStickyNotice(stickyNotice);
    }
    ...
    function setTransientNotice(keyOrMessage, kind, params) {
        if (typeof app.setNotice !== 'function') return;
        app.setNotice(t(String(keyOrMessage || ''), params || {}), kind || 'info');
    }
    ...
    applySeoMeta(t('login_title'), t('login_desc'));
    app.render(app.pageShell(t('login_title'), t('login_desc'), bodyHtml, footerHtml));

    const initialMessageCode = app.pageMsg || '';
    const initialMessage = GOOGLE_MESSAGE_MAP[initialMessageCode];
    if (initialMessage) {
        if (initialMessageCode.indexOf('BOT_') === 0) setStickyNotice(initialMessage.key, initialMessage.kind || 'error', { support_email: app.supportEmail });
        else setTransientNotice(initialMessage.key, initialMessage.kind || 'error');
    }

    const emailEl = document.getElementById('txt_email');
    const passwordEl = document.getElementById('txt_password');
    if (emailEl) {
        emailEl.addEventListener('input', function () { emailEl.dataset.touched = '1'; clearTransientNotice(); updateEmailHelper(false); });
        emailEl.addEventListener('blur', function () { emailEl.dataset.touched = '1'; updateEmailHelper(true); });
    }
    if (passwordEl) {
        passwordEl.addEventListener('input', function () { passwordEl.dataset.touched = '1'; clearTransientNotice(); updatePasswordHelper(false); });
    ...
    app.bindSubmit('account_login_form', async function () {
        ...
        try {
            ...
            const checker = String(response.checker || '').trim().toUpperCase();
            if (checker === 'SUCCESS') {
                clearStickyNotice();
                setTransientNotice(t('login_success'), 'success');
                try { sessionStorage.removeItem(RETURN_TO_KEY); } catch (error) {}
                app.redirect(getReturnToUrl());
                return;
            }
            if (checker === 'WRONGPASSWORD') {
                clearStickyNotice();
                clearTransientNotice();
            ...
            if (checker === 'REQUIRES_BOT_VERIFICATION') {
                const mailStatus = String(response.verification_mail_status || '').trim().toLowerCase();
                if (mailStatus === 'rate_limited') {
                    setStickyNotice('bot_verification_rate_limited', 'error', { support_email: app.supportEmail });
                    showFieldMessage('txt_email', t('bot_verification_rate_limited_help'), 'info');
                } else {
                    setStickyNotice('bot_verification_required', 'error', { support_email: app.supportEmail });
                    showFieldMessage('txt_email', t('bot_verification_sent'), 'info');
                }
                clearFieldMessage('txt_password');
                return;
            }
            if (checker === 'BOT_VERIFICATION_SEND_FAILED') {
                setStickyNotice('bot_verification_send_failed', 'error', { support_email: response.support_email || app.supportEmail });
                showFieldMessage('txt_email', t('bot_verification_send_failed_help'), 'error');
                clearFieldMessage('txt_password');
                return;
            }
            clearStickyNotice();
            setTransientNotice(response.checker || t('generic_error'), 'error');
        } catch (error) {
