window.set_main = function () {
    ...
    function renderLoading() {
        app.render(app.pageShell(
            app.t('myinfo_title'),
            app.t('myinfo_desc'),
            '<div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">' + app.t('loading') + '</div>',
            ''
        ));
    }
    ...
    function infoBadge(label, value) {
        return [
            ...
            '  <div class="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">' + app.escapeHtml(label) + '</div>',
            '  <div class="mt-2 text-sm font-medium text-slate-800 dark:text-slate-100">' + app.escapeHtml(value || '-') + '</div>',
            '</div>'
        ].join('');
    }
    ...
    function renderForm(userinfo) {
        const summaryHtml = [
            ...
            infoBadge(app.t('role'), userinfo.role || '-'),
            infoBadge(app.t('date_joined'), userinfo.date_joined || '-'),
            infoBadge(app.t('officer'), app.readableBool(userinfo.officer || userinfo.is_current_officer || 0)),
            '</div>'
        ].join('');

        const formHtml = [
            summaryHtml,
            '<form id="account_myinfo_form" class="mt-6 space-y-6">',
        ...
        const footerHtml = '<div class="text-xs text-slate-400 dark:text-slate-500">' + app.t('profile_note') + '</div>';
        app.render(app.pageShell(app.t('myinfo_title'), app.t('myinfo_desc'), formHtml, footerHtml));
        ...
        try {
            ...
            if (response.checker === 'UNAUTHORIZED') {
                app.render(app.pageShell(
                    app.t('myinfo_title'),
                    app.t('myinfo_desc'),
                    '<div class="space-y-4"><div class="rounded-2xl border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-200">' + app.t('unauthorized') + '</div><a href="' + app.routes.login + '" class="inline-flex h-12 items-center justify-center rounded-2xl bg-slate-900 px-5 text-sm font-semibold text-white transition hover:bg-slate-700 dark:bg-sky-500 dark:text-slate-950 dark:hover:bg-sky-400">' + app.t('go_login') + '</a></div>',
                    ''
                ));
                return;
            }
