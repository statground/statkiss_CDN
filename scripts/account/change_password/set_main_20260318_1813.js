window.set_main = function () {
    ...
    function renderInvalidToken() {
        const bodyHtml = [
            ...
            '       <a href="' + app.routes.changePassword + '" class="inline-flex h-12 items-center justify-center rounded-2xl bg-slate-900 px-5 text-sm font-semibold text-white transition hover:bg-slate-700 dark:bg-sky-500 dark:text-slate-950 dark:hover:bg-sky-400">' + app.t('retry') + '</a>',
            '       <a href="' + app.routes.login + '" class="inline-flex h-12 items-center justify-center rounded-2xl border border-slate-300 px-5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900">' + app.t('go_login') + '</a>',
            '   </div>',
            '</div>'
        ].join('');
