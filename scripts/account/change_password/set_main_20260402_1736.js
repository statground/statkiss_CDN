window.set_main = function () {
    ...
    function resolveCurrentLanguage() {
        const I = window.StatKISS_I18N;
        const fallback = app.lang || 'en';
        if (I && typeof I.resolveLangCode === 'function') {
            return I.resolveLangCode(fallback);
        }
        return fallback;
    }

    function languageFieldLabel() {
        const fallbackMap = {
            ko: '언어',
            ja: '言語',
            'zh-Hans': '语言',
        ...
        if (I && typeof I.t === 'function') {
            const translated = I.t(lang, 'lang.title');
            if (translated && translated !== 'lang.title') {
                return translated;
            }
        }
    ...
    function languageOptions() {
        return getLanguageBundle().map(function (item) {
    ...
    function renderRequestForm() {
        ...
        const bodyHtml = [
            ...
            '  <div>' + app.selectInput('email_lang', languageFieldLabel(), languageOptions(), currentLang) + '</div>',
        ...
        app.bindSubmit('password_request_form', async function () {
            ...
            const selectedLang = app.valueOf('email_lang') || currentLang;
            ...
            try {
                ...
                const response = await app.requestJSON(app.api.sendAuthEmail, app.createFormData({ email: email, lang: selectedLang }));
                ...
                app.setNotice(app.t('generic_error'), 'error');
            } catch (error) {
