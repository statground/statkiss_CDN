(function () {
    ...
    const routes = {
        ...
        myinfoConnections: (bootstrap.routes && bootstrap.routes.myinfoConnections) || pathFromBase('myinfo/connections/'),
        ...
        googleLogin: (bootstrap.routes && bootstrap.routes.googleLogin) || pathFromBase('google/login/'),
        googleSignup: (bootstrap.routes && bootstrap.routes.googleSignup) || pathFromBase('google/signup/'),
        googleLink: (bootstrap.routes && bootstrap.routes.googleLink) || pathFromBase('google/link/'),
...
const ACCOUNT_THEME_ATTRS = ['data-theme', 'theme', 'data-color-mode', 'data-bs-theme', 'color-scheme'];
...
function readThemeSignalFromElement(element) {
    ...
    if (/dark/.test(className) || /theme-dark/.test(className)) return 'dark';
    if (/light/.test(className) || /theme-light/.test(className)) return 'light';
    return '';
}
...
function observeAccountThemeChanges() {
    ...
    if (typeof MutationObserver === 'function') {
        ...
        const observeTarget = (target) => {
            if (!target) return;
            observer.observe(target, { attributes: true, attributeFilter: ['class', 'style', 'data-theme', 'theme', 'data-color-mode', 'data-bs-theme', 'color-scheme'] });
        };
