...
function getDisplayDomain(rawUrl) {
    ...
    function getForumsBasePath() {
        var config = getUsefulLinksConfig();

        if (config.basePath) {
            var configuredBase = normalizePath(stripQueryAndHash(config.basePath));
            return configuredBase.replace(/\/?$/, '/');
        }

        var pathname = '/';
        try {
            pathname = stripQueryAndHash(window.location.pathname || '/');
        } catch (error) {}
    ...
    function buildAjaxUrl(action) {
        var config = getUsefulLinksConfig();
        var directMap = {
            get_links: config.ajaxGetUrl,
            add_links: config.ajaxAddUrl,
            delete_links: config.ajaxDeleteUrl
        };

        if (directMap[action]) {
            return directMap[action];
        }

        return getForumsBasePath().replace(/\/?$/, '/') + 'ajax_' + action + '/';
    }
...
function buildCardsHtml() {
    ...
    function deleteLink(uuid) {
        ...
        fetch(buildAjaxUrl('delete_links'), {
            method: 'POST',
            credentials: 'same-origin',
            headers: {
                'X-CSRFToken': getCookie('csrftoken')
