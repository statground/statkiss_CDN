(function () {
    ...
    var DEFAULT_CATEGORY_META = [
        { key: 'category.academic_society', defaultValue: 'Academic Society' },
        { key: 'category.government_institutions', defaultValue: 'Government Institutions' },
        { key: 'category.ngo', defaultValue: 'NGO' },
        { key: 'category.financial_institutions', defaultValue: 'Financial Institutions' },
        { key: 'category.educational_institutions', defaultValue: 'Educational Institutions' },
        { key: 'category.healthcare_institutions', defaultValue: 'Healthcare Institutions' },
        { key: 'category.research_institutions', defaultValue: 'Research Institutions' },
        { key: 'category.cultural_and_arts_institutions', defaultValue: 'Cultural and Arts Institutions' },
        { key: 'category.private_company', defaultValue: 'Private Company' },
        { key: 'category.business_corporation', defaultValue: 'Business Corporation' }
    ];
    ...
    var CARD_TONES = [
        {
            shell: 'from-slate-50 via-white to-blue-50/60',
        ...
        {
            shell: 'from-emerald-50/70 via-white to-teal-50/70',
        ...
        {
            shell: 'from-violet-50/70 via-white to-fuchsia-50/70',
        ...
        {
            shell: 'from-amber-50/80 via-white to-orange-50/70',
    ...
    for (var i = 0; i < DEFAULT_CATEGORY_META.length; i += 1) {
        ...
        CATEGORY_KEY_BY_NORMALIZED_VALUE[normalizeValue(DEFAULT_CATEGORY_META[i].defaultValue)] = DEFAULT_CATEGORY_META[i].key;
    }
    ...
    function buildLoadingHtml() {
        var card = [
            '<div class="overflow-hidden rounded-[1.75rem] border border-gray-200 bg-white p-6 shadow-sm">',
                '<div class="mb-5 flex items-center justify-between">',
                    '<div class="h-7 w-28 rounded-full bg-gray-100"></div>',
    ...
    function buildCardsHtml() {
        ...
        if (!state.displayLinks.length) {
            return buildEmptyHtml();
        }

        var html = '<div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">';

        for (var i = 0; i < state.displayLinks.length; i += 1) {
            var item = state.displayLinks[i];
            var tone = CARD_TONES[i % CARD_TONES.length];
            var hasDescription = !!String(item.description || '').trim();
            var domainText = item.hostname || item.url;

            html += '<article class="group relative overflow-hidden rounded-[1.75rem] border border-gray-200 bg-gradient-to-br ' + tone.shell + ' p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">';
            html += '<div class="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ' + tone.accent + '"></div>';
            html += '<div class="flex items-start justify-between gap-3">';
            html += '<span class="inline-flex max-w-[75%] items-center rounded-full px-3.5 py-1.5 text-xs font-semibold ring-1 ' + tone.badge + '">' + escapeHtml(localizeCategory(item.category)) + '</span>';

            if (state.canManage) {
                html += '<button type="button" data-action="delete-link" data-uuid="' + escapeHtml(item.uuid) + '" class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-transparent bg-white/80 text-gray-400 shadow-sm transition hover:border-red-100 hover:bg-red-50 hover:text-red-600" aria-label="' + escapeHtml(t('useful_links.delete')) + '" title="' + escapeHtml(t('useful_links.delete')) + '">';
                ...
                html += '<span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/80 text-gray-300 shadow-sm">';
            ...
            html += '<div class="mt-6">';
            html += '<a href="' + escapeHtml(item.url) + '" target="_blank" rel="noreferrer noopener" class="block" aria-label="' + escapeHtml(t('useful_links.open_external')) + '">';
            html += '<h3 class="text-xl font-bold tracking-tight text-gray-900 transition group-hover:text-gray-700">' + escapeHtml(item.title) + '</h3>';
            if (domainText) {
                html += '<p class="mt-3 text-xs font-semibold uppercase tracking-[0.2em] ' + tone.domain + '">' + escapeHtml(domainText) + '</p>';
            }
            if (hasDescription) {
                html += '<p class="mt-4 text-sm leading-6 text-gray-600">' + escapeHtml(item.description) + '</p>';
            } else {
                html += '<div class="mt-4 h-[3.5rem]"></div>';
            }
            html += '</a>';
            html += '</div>';

            html += '<div class="mt-6 flex items-center justify-between gap-4">';
            html += '<span class="text-xs font-medium uppercase tracking-[0.18em] text-gray-400">#' + (i + 1) + '</span>';
            html += '<a href="' + escapeHtml(item.url) + '" target="_blank" rel="noreferrer noopener" class="inline-flex h-11 w-11 items-center justify-center rounded-full shadow-sm transition ' + tone.button + '" aria-label="' + escapeHtml(t('useful_links.open_external')) + '">';
            html += '<svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>';
            html += '</a>';
            html += '</div>';
            html += '</article>';
    ...
    function reloadLinks() {
        ...
        return fetchLinks().then(function (payload) {
            ...
            state.displayLinks = shuffleLinks(state.links);
