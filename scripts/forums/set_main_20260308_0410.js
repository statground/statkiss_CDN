...
function getDisplayDomain(rawUrl) {
    ...
    function buildErrorHtml() {
        ...
        return '<div class="mx-auto mb-8 max-w-screen-lg rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">' + escapeHtml(t('useful_links.load_error')) + '</div>';
    }
...
function buildCardsHtml() {
    ...
    for (var i = 0; i < state.links.length; i += 1) {
        ...
        html += '<article class="group relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">';
        ...
        html += '<h3 class="text-2xl font-bold tracking-tight text-slate-900 transition ' + tone.title + '">' + escapeHtml(item.title) + '</h3>';
        html += '</a>';
        if (domain) {
            html += '<p class="mt-5 text-sm font-semibold uppercase tracking-[0.24em] ' + tone.domain + '">' + escapeHtml(domain) + '</p>';
        }
        if (item.description) {
            html += '<p class="mt-4 text-sm leading-6 text-slate-600">' + escapeHtml(item.description) + '</p>';
        }
        html += '</div>';

        html += '<div class="mt-auto flex items-end justify-between gap-4 pt-7">';
        html += '<span class="text-sm font-semibold tracking-[0.18em] text-slate-400">#' + (i + 1) + '</span>';
        html += '<div class="flex items-center gap-3">';

        if (state.canManage) {
            html += '<button type="button" data-action="delete-link" data-uuid="' + escapeHtml(item.uuid) + '" class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-transparent text-slate-400 transition hover:border-red-100 hover:bg-red-50 hover:text-red-600" aria-label="' + escapeHtml(t('useful_links.delete')) + '" title="' + escapeHtml(t('useful_links.delete')) + '">';
