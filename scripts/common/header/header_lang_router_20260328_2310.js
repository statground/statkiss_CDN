(function () {
  ...
  function getSavedLang() {
    ...
    try {
      if (window.__STATKISS_LANG__) return normalizeLang(window.__STATKISS_LANG__);
    } catch (_) {}
    const htmlLang = document.documentElement.getAttribute('lang');
    if (htmlLang) return normalizeLang(htmlLang);
  ...
  function maybeMoveCurrentPage(lang) {
    if (!isLangRoutablePage(window.location.pathname)) return false;
  ...
  function bind() {
    apply(getSavedLang(), false);

    function syncCurrentPage(forceNavigate) {
      const nextLang = getSavedLang();
      apply(nextLang, false);
      if (forceNavigate) {
        return maybeMoveCurrentPage(nextLang);
      }
      if (isLangRoutablePage(window.location.pathname)) {
        const currentParts = String(window.location.pathname || '').split('/').filter(Boolean);
        const supported = getSupportedLangs();
        const pathLang = (currentParts.length && supported.includes(currentParts[0])) ? normalizeLang(currentParts[0]) : '';
        if (nextLang && pathLang && nextLang !== pathLang) {
          return maybeMoveCurrentPage(nextLang);
        }
      }
      return false;
    ...
    window.addEventListener('statkiss:lang', function (e) {
      const nextLang = e && e.detail ? e.detail.lang : getSavedLang();
      apply(nextLang, false);
      maybeMoveCurrentPage(nextLang);
    });
    ...
    const rootObserver = new MutationObserver(function () {
      syncCurrentPage(false);
    });
    rootObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['lang', 'dir'] });
    ...
    syncCurrentPage(false);
    setTimeout(function () { syncCurrentPage(false); }, 0);
    setTimeout(function () { syncCurrentPage(false); }, 150);
  }
