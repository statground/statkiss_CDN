...
(function () {
  ...
  function getLocalePrefixCandidates() {
    ...
    try {
      ...
      if (match && match[1]) pushPrefix(`/${match[1]}`);
    } catch (_) {}
  ...
  function buildAccountPath(path) {
    ...
    const preferredPrefix = prefixes.find(Boolean) || '';
    if (!preferredPrefix) return normalizedPath;
    if (normalizedPath === '/') return `${preferredPrefix}/`;
    return `${preferredPrefix}${normalizedPath}`;
  }
