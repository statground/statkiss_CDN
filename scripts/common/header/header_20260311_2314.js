...
(function () {
  ...
  function normalizeHeaderUser(payload) {
    ...
    const legacyUsername = String(readLegacyGlobal('gv_username') || '').trim();
    const legacyName = String(readLegacyGlobal('gv_name') || readLegacyGlobal('gv_user_name') || '').trim();
    const legacyRole = String(readLegacyGlobal('gv_role') || '').trim();
    const legacyOfficer = String(readLegacyGlobal('gv_officer') || '').trim();
    const legacyJoint = toIntFlag(readLegacyGlobal('gv_role_addon_kssjoint'));

    const username = String(data?.username ?? data?.gv_username ?? legacyUsername ?? '').trim();
    const name = String(data?.name ?? legacyName ?? '').trim();
    const role = String(data?.role ?? data?.gv_role ?? legacyRole ?? '').trim();
    const officer = String(data?.officer ?? legacyOfficer ?? 'Member').trim() || 'Member';
    const role_addon_kssjoint = toIntFlag(data?.role_addon_kssjoint ?? legacyJoint);

    const explicitAuth = data?.is_authenticated ?? data?.authenticated ?? data?.is_login ?? data?.is_logged_in ?? data?.logged_in;
    const explicitAuthenticated = explicitAuth === true || explicitAuth === 1 || explicitAuth === '1' || explicitAuth === 'true';
    const authenticated = !!(
      explicitAuthenticated ||
      username ||
      legacyUsername ||
      (name && (role || officer !== 'Member' || role_addon_kssjoint === 1))
    );
    ...
    return {
      ...
      officer: authenticated ? officer : 'Member',
      role_addon_kssjoint: authenticated ? role_addon_kssjoint : 0,
    };
  }
  ...
  function HeaderAccountLinks({ user, t }) {
    ...
    return (
      <div className="flex flex-wrap items-center justify-end gap-3">
        ...
        <HeaderDivider />
        <HeaderInlineLink href="/membership/" strong>{safeUser.role || t('auth.membership')}</HeaderInlineLink>

        {safeUser.officer && safeUser.officer !== 'Member' ? (
          <>
            <HeaderDivider />
            <HeaderInlineLink strong>{safeUser.officer}</HeaderInlineLink>
          </>
        ) : null}

        {safeUser.role_addon_kssjoint === 1 ? (
          <>
            <HeaderDivider />
            <HeaderInlineLink strong>KSS Joint Member</HeaderInlineLink>
          </>
        ) : null}
