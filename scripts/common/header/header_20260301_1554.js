      ...
      return (
        <div className="relative" ref={ref}>
          <button
            ...
            className="statkiss-btn inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold shadow-sm hover:bg-slate-50 text-slate-700 dark:text-slate-100"
    ...
    function ThemeButton() {
      return (
        <button
          ...
          className="statkiss-btn inline-flex items-center justify-center rounded-full border border-slate-200 bg-white p-2 shadow-sm hover:bg-slate-50 text-slate-700 dark:text-slate-100"
        ...
        <div className="flex items-center gap-2">
          ...
          {user.loaded && !user.username && (
            <div className="flex items-center gap-2 text-xs">
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 text-slate-700 dark:text-slate-50 dark:hover:bg-slate-800/70" href="/account/">{t('auth.login')}</a>
              <span className="statkiss-divider text-slate-300">|</span>
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 text-slate-700 dark:text-slate-50 dark:hover:bg-slate-800/70" href="/account/signup/">{t('auth.signup')}</a>
            </div>
          )}
          ...
          {user.loaded && user.username && (
            <div className="flex items-center gap-2 text-xs">
              ...
              {isAdminLike() && (
                <>
                  <span className="statkiss-divider text-slate-300">|</span>
                  <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 text-slate-700 dark:text-slate-50 dark:hover:bg-slate-800/70" href="/admin/">{t('auth.admin')}</a>
                </>
              )}

              <span className="statkiss-divider text-slate-300">|</span>
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 text-slate-700 dark:text-slate-50 dark:hover:bg-slate-800/70" href="/account/logout/">{t('auth.logout')}</a>
            </div>
          )}
        </div>
      );
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
                    {item.cols.map((col, idx) => (
                      <div key={idx} className="space-y-2">
                        {col.map((link, j) => (
                          <a key={j} href={link.href} target={link.target || '_self'} className="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 hover:bg-slate-100 dark:text-slate-50 dark:hover:bg-slate-800/70">
                            {link.label}
                          </a>
                        ))}
                      </div>
        ...
        <div className="statkiss-header__main bg-white">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
            <a href="/" className="flex items-center">
              <img src="https://cdn.jsdelivr.net/gh/statground/statkiss_CDN/images/logo/logo_transparent.png" className="h-12 md:h-14 w-auto drop-shadow-[0_2px_10px_rgba(0,0,0,0.10)] dark:drop-shadow-[0_2px_24px_rgba(255,255,255,0.35)] dark:brightness-110 dark:contrast-110" alt="StatKISS" />
            </a>
