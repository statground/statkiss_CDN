...
(function () {
  ...
  function pushUnique(items, value) {
    const next = toText(value).replace(/\/+$|^\s+|\s+$/g, '');
    if (!next) return;
  ...
  function getLocalePrefixes() {
    const prefixes = [''];
    const pathname = toText(typeof window !== 'undefined' ? window.location.pathname : '');
    const pathMatch = pathname.match(/^\/([A-Za-z]{2}(?:-[A-Za-z]{2})?)(?=\/|$)/);
    if (pathMatch) pushUnique(prefixes, `/${pathMatch[1]}`);

    const htmlLang = toText(typeof document !== 'undefined' ? document.documentElement.getAttribute('lang') : '');
    if (htmlLang) {
      pushUnique(prefixes, `/${htmlLang}`);
      if (htmlLang.includes('-')) pushUnique(prefixes, `/${htmlLang.split('-')[0]}`);
    }

    try {
      const initialLang = toText(getInitialLang());
      if (initialLang) {
        pushUnique(prefixes, `/${initialLang}`);
        if (initialLang.includes('-')) pushUnique(prefixes, `/${initialLang.split('-')[0]}`);
      }
    } catch (_) {}
  ...
  function buildHeaderApiCandidates() {
    const candidates = [];
    const accountPaths = [
      '/account/ajax_get_menu_header/',
      '/account/ajax_get_menu_header',
      '/account/ajax_get_userinfo/',
      '/account/ajax_get_userinfo',
    ];
    const rootPaths = [
      '/ajax_get_menu_header/',
      '/ajax_get_menu_header',
      '/ajax_get_userinfo/',
      '/ajax_get_userinfo',
    ];

    getLocalePrefixes().forEach((prefix) => {
      accountPaths.forEach((path) => pushUnique(candidates, `${prefix}${path}`));
    });
    rootPaths.forEach((path) => pushUnique(candidates, path));

    return candidates;
  }

  function normalizeHeaderUser(payload) {
    const data = payload || {};
    ...
    const username = toText(data?.username ?? data?.uuid ?? data?.gv_username ?? legacyUsername ?? '');
    const name = toText(data?.name ?? data?.first_name ?? legacyName ?? '');
    const role = toText(data?.role ?? data?.gv_role ?? legacyRole ?? '');
    const role_addon = normalizeRoleAddon(data?.role_addon ?? data?.role_addon_kssjoint ?? legacyRoleAddon);
    const current_officer_role = normalizeOfficerRole(
      data?.current_officer_role ?? data?.current_officer_roles ?? legacyOfficerRole
    );
    const is_current_officer = toIntFlag(
      data?.is_current_officer ?? data?.current_officer ?? data?.gv_is_current_officer ?? legacyOfficerFlag ?? (current_officer_role ? 1 : 0)
    );
    ...
    return {
      loaded: true,
      authenticated,
      username: authenticated ? (username || name || legacyUsername || '') : '',
      name: authenticated ? (name || username || legacyName || '') : '',
      role: authenticated ? role : '',
      role_addon: authenticated ? role_addon : 'Non-member',
      role_addon_kssjoint: authenticated ? (hasVisibleRoleAddon(role_addon) ? 1 : 0) : 0,
      ...
      can_admin_page: authenticated ? (data?.can_admin_page === true || data?.can_admin_page === 1 || data?.can_admin_page === '1' || data?.can_admin_page === 'true') : false,
    };
  }

  async function fetchHeaderUser() {
    const candidates = buildHeaderApiCandidates();
    let lastPayload = {};
    ...
    for (const url of candidates) {
      try {
        ...
        lastPayload = data || {};
        if (data?.checker && data.checker !== 'SUCCESS') continue;
        return normalizeHeaderUser(data);
      } catch (_) {}
    }

    return normalizeHeaderUser(lastPayload);
  }

  function isAdminLikeUser(user) {
    if (!user || !user.authenticated) return false;
    const r = user.role;
    return !!(user.can_admin_page || user.is_current_officer === 1 || r === 'Administrator' || r === 'Developer');
  }
  ...
  function HeaderAccountLinks({ user, t }) {
    const safeUser = user || normalizeHeaderUser({});
    const roleLabel = (() => {
      const baseRole = safeUser.role || t('auth.membership');
      const addon = hasVisibleRoleAddon(safeUser.role_addon) ? ` (${safeUser.role_addon})` : '';
      return `${baseRole}${addon}`;
    })();

    if (!safeUser.authenticated) {
      return (
        <div className="flex flex-wrap items-center justify-end gap-3">
          <HeaderInlineLink href="/account/">{t('auth.login')}</HeaderInlineLink>
          <HeaderDivider />
          <HeaderInlineLink href="/account/signup/">{t('auth.signup')}</HeaderInlineLink>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-wrap items-center justify-end gap-3">
        ...
        <HeaderDivider />
        <HeaderInlineLink href="/membership/" strong>{roleLabel}</HeaderInlineLink>

        {safeUser.is_current_officer === 1 && safeUser.current_officer_role ? (
          <>
            <HeaderDivider />
            <HeaderInlineLink strong>{safeUser.current_officer_role}</HeaderInlineLink>
          </>
        ) : null}

        {isAdminLikeUser(safeUser) ? (
          <>
            <HeaderDivider />
            <HeaderInlineLink href="/admin/">{(t('auth.admin') && t('auth.admin') !== 'auth.admin') ? t('auth.admin') : 'Admin Page'}</HeaderInlineLink>
          </>
        ) : null}

        <HeaderDivider />
        <HeaderInlineLink href="/account/logout/">{t('auth.logout')}</HeaderInlineLink>
      </div>
    );
  }
  ...
  function LegacySubHeader() {
    ...
    const [user, setUser] = useState(() => normalizeHeaderUser({}));
