...
(function () {
  ...
  function getUserinfoEndpointCandidates() {
    const basePaths = [
    ...
    try {
      const bootstrap = window.__ACCOUNT_BOOTSTRAP__ || window.ACCOUNT_BOOTSTRAP || window.bootstrap || {};
      const api = bootstrap.api || {};
      push(api.getUserinfo);
      push(api.get_userinfo);
    } catch (_) {}
  ...
  function normalizeHeaderUser(payload) {
    ...
    const name = toText(data?.name ?? legacyName ?? '');
  ...
  async function fetchHeaderUser() {
    const candidates = getUserinfoEndpointCandidates();
    let lastPayload = {};
    ...
    return (
      <div className="flex flex-wrap items-center justify-end gap-3">
        ...
        {isAdminLikeUser(safeUser) ? (
          <>
            <HeaderDivider />
            <HeaderInlineLink href="/admin/">Admin Page</HeaderInlineLink>
          </>
        ) : null}
