(function () {
  ...
  function syncCurrentPageWithSavedLang() {
    const saved = getSavedLang();
    if (!saved) return;
    if (!isLangRoutablePage(window.location.pathname)) return;

    const currentUrl = window.location.pathname + window.location.search + window.location.hash;
    const nextUrl = buildCurrentPageLangUrl(saved);
  ...
  function bind() {
    ...
    window.addEventListener('storage', function (e) {
      const I = getI18N();
      if (!I || e.key !== I.LANG_KEY) return;
      const nextLang = e.newValue ? normalizeLang(e.newValue) : getCurrentLang();
      apply(nextLang, true);
    });

    const htmlObserver = new MutationObserver(function () {
      const htmlLang = document.documentElement.getAttribute('lang');
      if (htmlLang) apply(htmlLang, true);
    });
