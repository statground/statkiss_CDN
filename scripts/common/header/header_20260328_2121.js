  ...
  function persistLangPreference(nextLang) {
    try {
      ...
      const normalized = (I && typeof I.resolveLangCode === 'function')
        ? I.resolveLangCode(nextLang || 'en')
        : (String(nextLang || 'en').trim() || 'en');
      localStorage.setItem(langKey, normalized);
      return normalized;
    } catch (_) {
      return String(nextLang || 'en').trim() || 'en';
    }
  }
            ...
            <div className="statkiss-card absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700/60 dark:bg-slate-900/40 shadow-lg">
              ...
              <div className="max-h-72 overflow-auto">
                {langs.map(l => {
                  ...
                  const targetHref = buildLocalizedCurrentUrl(l.code);
                  return (
                    <a
                      key={l.code}
                      href={targetHref}
                      data-statkiss-lang-link="1"
                      data-statkiss-lang-code={l.code}
                      hrefLang={l.code}
                      onClick={() => {
                        persistLangPreference(l.code);
                      }}
                      className={cx('statkiss-hover block w-full px-4 py-3 text-left text-sm hover:bg-slate-50', active ? 'statkiss-langitem--active bg-slate-50' : '')}
                    >
