                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
                    {item.cols.map((col, idx) => (
                      <div key={idx} className="space-y-2">
                        {col.map((link, j) => (
                          <a key={j} href={link.href} target={link.target || '_self'} className="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 hover:bg-slate-100 dark:text-slate-100 dark:hover:bg-slate-800">
                            {link.label}
                          </a>
                        ))}
                      </div>
        ...
        <div className="statkiss-header__main bg-white">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
            <a href="/" className="flex items-center">
              <img src="https://cdn.jsdelivr.net/gh/statground/statkiss_CDN/images/logo/logo_transparent.png" className="h-12 md:h-14 w-auto drop-shadow-[0_2px_10px_rgba(0,0,0,0.10)] dark:drop-shadow-[0_2px_14px_rgba(255,255,255,0.18)]" alt="StatKISS" />
            </a>
