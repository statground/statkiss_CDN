...
(function () {
  ...
  function getCurrentLangCode() {
    try {
      ...
      if (I && typeof I.resolveLangCode === 'function') {
        const fromPath = getLocalePrefixFromPathname().replace(/^\//, '');
        let fromStorage = '';
        try { fromStorage = localStorage.getItem(I.LANG_KEY) || ''; } catch (_) {}
        ...
        const resolved = I.resolveLangCode(fromPath || fromStorage || fromWindow || fromDocument || fromNavigator || 'en');
        return String(resolved || 'en').trim() || 'en';
      }
    } catch (_) {}

    try {
      const lang = getLocalePrefixFromPathname().replace(/^\//, '') || document?.documentElement?.getAttribute('lang');
  ...
  function moveToLocalizedCurrentPage(nextLang, options) {
    ...
    if (nextUrl && nextUrl !== currentUrl) {
      const opts = options || {};
      try { window.__STATKISS_PENDING_LANG_NAV__ = nextUrl; } catch (_) {}
      try {
      ...
      setTimeout(function () {
        try {
          const now = String(window.location.pathname || '/') + String(window.location.search || '') + String(window.location.hash || '');
          if (now !== nextUrl) window.location.href = nextUrl;
        } catch (_) {}
      }, 120);
      return true;
    }
  ...
  function forceSwitchLanguage(nextLang) {
    const I = window.StatKISS_I18N;
    const normalized = I && typeof I.resolveLangCode === 'function'
      ? I.resolveLangCode(nextLang)
      : (String(nextLang || '').trim() || 'en');

    try { window.__STATKISS_LANG__ = normalized; } catch (_) {}
    try { localStorage.setItem((I && I.LANG_KEY) || 'statkiss_lang', normalized); } catch (_) {}
    try {
      if (I && typeof I.applyLangToDocument === 'function') I.applyLangToDocument(normalized);
      else document.documentElement.setAttribute('lang', normalized);
    } catch (_) {}

    const moved = moveToLocalizedCurrentPage(normalized);
                ...
                {langs.map(l => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setOpen(false);
                        const moved = forceSwitchLanguage(l.code);
                        if (!moved) {
                          setLang(l.code);
                        }
                      }}
