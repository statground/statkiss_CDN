...
(function () {
  ...
  function Header() {
      ...
      return (
        <div className="relative" ref={ref}>
          <button
            ...
            className="statkiss-btn inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold shadow-sm hover:bg-slate-50"
          ...
          {open && (
            <div className="statkiss-card absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
    ...
    function ThemeButton() {
      return (
        <button
          ...
          className="statkiss-btn inline-flex items-center justify-center rounded-full border border-slate-200 bg-white p-2 shadow-sm hover:bg-slate-50"
        ...
        <div className="flex items-center gap-2">
          ...
          {user.loaded && !user.username && (
            <div className="flex items-center gap-2 text-xs">
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800" href="/account/">{t('auth.login')}</a>
              <span className="statkiss-divider text-slate-300">|</span>
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800" href="/account/signup/">{t('auth.signup')}</a>
            </div>
          )}
          ...
          {user.loaded && user.username && (
            <div className="flex items-center gap-2 text-xs">
              <a className="flex items-center gap-2 rounded-full px-3 py-2 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800" href="/account/myinfo/">
                <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-900 text-white text-[11px] font-extrabold">{(user.name || user.username || 'U').slice(0,1).toUpperCase()}</span>
              ...
              {isAdminLike() && (
                <>
                  <span className="statkiss-divider text-slate-300">|</span>
                  <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800" href="/admin/">{t('auth.admin')}</a>
                </>
              )}

              <span className="statkiss-divider text-slate-300">|</span>
              <a className="rounded-full px-3 py-2 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800" href="/account/logout/">{t('auth.logout')}</a>
            </div>
          )}
        </div>
      );
          ...
          {menu.map(item => (
            <button
              ...
              className={cx(
                ...
                openDesktop === item.key ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100'
              )}
            >
              {item.title}
            </button>
      ...
      return (
        <div className="absolute left-0 right-0 top-full z-50">
          <div className="mx-auto max-w-6xl px-4">
            <div ref={megaRef} className="statkiss-mega mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
              <div className="grid grid-cols-12 gap-0">
                <div className="col-span-12 md:col-span-3">
                  <div className="h-full p-6">
                    <div className="statkiss-card rounded-2xl bg-slate-50 p-6 border border-slate-100">
                      <div className={cx('text-lg font-extrabold bg-gradient-to-r bg-clip-text text-transparent', item.leftBg)}>{item.title}</div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
                    {item.cols.map((col, idx) => (
                      <div key={idx} className="space-y-2">
                        {col.map((link, j) => (
                          <a key={j} href={link.href} target={link.target || '_self'} className="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 hover:bg-slate-100">
                            {link.label}
                          </a>
                        ))}
                      </div>
                ...
                <div className="col-span-12 md:col-span-3">
                  <div className="p-6">
                    {item.image ? (
                      ...
                      <div className="h-40 w-full rounded-2xl border border-dashed border-slate-300" />
                    )}
                  </div>
                </div>
              </div>
    ...
    function MobileMenu() {
      return (
        <div className="mx-auto max-w-6xl px-4">
          <div className="statkiss-card mt-3 rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="p-3">
              {menu.map(item => (
        ...
        <div className="statkiss-header__main bg-white">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
            <a href="/" className="flex items-center">
              <img src="https://cdn.jsdelivr.net/gh/statground/statkiss_CDN/images/logo/logo.png" className="h-12 md:h-14 w-auto" alt="StatKISS" />
            </a>
            ...
            {isDesktop ? (
              ...
              <button
                type="button"
                className="statkiss-btn inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold shadow-sm hover:bg-slate-50"
