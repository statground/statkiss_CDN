function set_main() {
  ...
  (function injectPolicyThemeFallback() {
    if (document.getElementById('statkiss-policy-fallback-' + "StatKISS_I18N_TERMS_OF_SERVICE")) return;

    const style = document.createElement('style');
    style.id = 'statkiss-policy-fallback-' + "StatKISS_I18N_TERMS_OF_SERVICE";
    style.textContent = `
  ...
  function getSource() {
    return window["StatKISS_I18N_TERMS_OF_SERVICE"] || null;
  }
  ...
  function getLocalizedPageUrl(nextLang) {
    const normalizedLang = resolveLang(nextLang || getLang());
    const pathname = String(window.location.pathname || '/');
    const search = String(window.location.search || '');
    const hash = String(window.location.hash || '');
    const parts = pathname.split('/').filter(Boolean);

    if (!parts.length) return `/${normalizedLang}/terms-of-service/` + search + hash;

    const H = window.StatKISS_I18N;
    const supported = (H && Array.isArray(H.languages)) ? H.languages.map((item) => item.code) : ['en'];
    const hasLangPrefix = supported.includes(parts[0]);

    if (hasLangPrefix) {
      parts[0] = normalizedLang;
      if (parts.length >= 2 && parts[1] !== 'terms-of-service') parts[1] = 'terms-of-service';
      return '/' + parts.join('/') + (pathname.endsWith('/') ? '/' : '/') + search + hash;
    }

    if (parts[0] !== 'terms-of-service') {
      return `/${normalizedLang}/terms-of-service/` + search + hash;
    }

    return `/${normalizedLang}/${parts.join('/')}` + (pathname.endsWith('/') ? '/' : '/') + search + hash;
  }

  function moveToLocalizedPage(nextLang) {
    const nextUrl = getLocalizedPageUrl(nextLang);
  ...
  function Div_main() {
    ...
    useEffect(() => {
      ...
      const onLang = (e) => {
        const nextLang = pickLangFromEvent(e);
        applyLang(nextLang);
        moveToLocalizedPage(nextLang);
      };
