function set_main() {
  ...
  const POLICY_NAMESPACE = 'StatKISS_I18N_PRIVACY_POLICY';
  const FALLBACK_SLUG = 'privacy-policy';
  ...
  function getSupportedLangs() {
    const H = window.StatKISS_I18N;
    if (H && Array.isArray(H.languages)) return H.languages.map((item) => item.code);
    const S = getSource();
  ...
  function getLang() {
    ...
    const desired = getDesiredLang();
    if (desired) return desired;

    const browser = navigator.language || 'en';
    return resolveLang(browser || 'en');
  }

  function getSavedLang() {
    const langKey = getLangKey();
    try {
      return localStorage.getItem(langKey) || localStorage.getItem('statkiss_lang') || '';
    } catch (_) {
      return '';
    }
  }
  ...
  function getDesiredLang() {
    ...
    const fromSaved = getSavedLang();
    const fromDoc = document.documentElement.getAttribute('lang') || '';
    const fromPath = getPathLang();
    return resolveLang(fromWindow || fromSaved || fromDoc || fromPath || 'en');
  }
  ...
  function buildPageUrl(lang) {
    return buildPagePath(lang) + String(window.location.search || '') + String(window.location.hash || '');
  }
  ...
  function moveToLocalizedPage(nextLang, options) {
    const nextUrl = buildPageUrl(nextLang);
    const currentUrl = getCurrentUrl();
    if (!nextUrl || nextUrl === currentUrl || pendingNavigation) {
      return false;
    }
  ...
  function Div_main() {
    ...
    useEffect(() => {
      ...
      const applyLang = (raw) => {
        const resolved = resolveLang(raw || getLang());
      ...
      const syncRouteToLang = (raw, options) => {
        const resolved = applyLang(raw);
        const pathLang = getPathLang();
        if (!pathLang || pathLang !== resolved) {
          moveToLocalizedPage(resolved, options || { replace: true });
        }
      };

      const onLang = (e) => {
        syncRouteToLang(pickLangFromEvent(e), { replace: false });
      };
      ...
      const onStorage = (e) => {
        if (!e) return;
        const langKey = getLangKey();
        if (e.key === langKey || e.key === 'statkiss_lang') {
          syncRouteToLang(e.newValue, { replace: true });
        }
      };
      ...
      const mo = new MutationObserver(() => {
        const docLang = document.documentElement.getAttribute('lang') || null;
        const pathLang = getPathLang();
        const resolved = applyLang(docLang || pathLang || null);
        if (docLang && (!pathLang || pathLang !== resolved)) {
          moveToLocalizedPage(resolved, { replace: true });
        }
      });
      ...
      const onFocusSync = () => syncRouteToLang(getDesiredLang(), { replace: true });
      ...
      let lastDesired = getDesiredLang();
      const watchdog = window.setInterval(() => {
        const desired = getDesiredLang();
        const pathLang = getPathLang();
        if (desired !== lastDesired || (pathLang && desired && pathLang !== desired)) {
          lastDesired = desired;
          syncRouteToLang(desired, { replace: true });
        }
      }, 250);

      syncRouteToLang(getDesiredLang(), { replace: true });
