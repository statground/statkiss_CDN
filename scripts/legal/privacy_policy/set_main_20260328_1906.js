function set_main() {
  ...
  function getLocalizedPageUrl(nextLang) {
    ...
    if (!parts.length) return `/${normalizedLang}/privacy-policy/` + search + hash;
    ...
    if (hasLangPrefix) {
      ...
      if (parts.length >= 2 && parts[1] !== 'privacy-policy') parts[1] = 'privacy-policy';
      return '/' + parts.join('/') + (pathname.endsWith('/') ? '/' : '/') + search + hash;
    }

    if (parts[0] !== 'privacy-policy') {
      return `/${normalizedLang}/privacy-policy/` + search + hash;
    }
