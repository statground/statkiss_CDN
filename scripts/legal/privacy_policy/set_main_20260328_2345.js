function set_main() {
  ...
  function buildLocalizedUrl(nextLang) {
    ...
    return '/' + lang + '/privacy-policy/' + String(window.location.search || '') + String(window.location.hash || '');
  }
  ...
  function updateHeadFromDocument(lang, documentData) {
    ...
    const absolute = window.location.origin + '/' + resolveLang(lang) + '/privacy-policy/';
