function set_main() {
    ...
    function parseBoardSyncPayload(rawValue) {
    ...
    function readBoardSyncPayloadFromStorage() {
        try {
            return parseBoardSyncPayload(localStorage.getItem(BOARD_SYNC_STORAGE_KEY));
        } catch (e) {
            return null;
        }
    }
    ...
    function normalizeBoardPayload(payload) {
        ...
        return {
            ...
            visible: visible ? 1 : 0,
            sections: sections,
            current_version_uuid: String(source.current_version_uuid || source.saved_version_uuid || ''),
    ...
    function App() {
        ...
        const [board, setBoard] = React.useState({ visible: 0, sections: [], current_version_uuid: '', updated_at: '' });
        const boardVersionRef = React.useRef('');

        React.useEffect(() => {
            boardVersionRef.current = String((board && board.current_version_uuid) || '');
        }, [board.current_version_uuid]);
        ...
        React.useEffect(() => {
            ...
            async function loadBoard(options) {
                ...
                if (!opts.force && now - lastRequestedAt < 500) return;
                lastRequestedAt = now;
                isLoading = true;
                try {
                    ...
                    if (!response.ok || (rawPayload && rawPayload.ok === false)) {
                    ...
                    setBoard(function (prev) {
                        if (prev && (prev.current_version_uuid || (prev.sections && prev.sections.length))) {
                            return prev;
                        }
                        return { visible: 0, sections: [], current_version_uuid: '', updated_at: '' };
                    });
                } finally {
                    isLoading = false;
                }
            ...
            function refreshBoardFromStorage(event) {
                if (event && event.type === 'storage' && event.key && event.key !== BOARD_SYNC_STORAGE_KEY) {
                    return;
                }
                const syncPayload = event && event.type === 'storage'
                    ? parseBoardSyncPayload(event.newValue)
                    : readBoardSyncPayloadFromStorage();
                const syncedVersionUuid = String((syncPayload && (syncPayload.current_version_uuid || syncPayload.saved_version_uuid || syncPayload.version_uuid)) || '').trim();
                if (syncedVersionUuid && syncedVersionUuid === boardVersionRef.current) {
                    return;
                }
            ...
            function handlePageShow(event) {
                loadBoard({ force: !event || !!event.persisted });
            }
            ...
            window.addEventListener('storage', refreshBoardFromStorage);
            window.addEventListener(BOARD_SYNC_EVENT_NAME, refreshBoardFromStorage);
            ...
            return () => {
                ...
                window.removeEventListener('storage', refreshBoardFromStorage);
                window.removeEventListener(BOARD_SYNC_EVENT_NAME, refreshBoardFromStorage);
