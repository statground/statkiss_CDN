...
function set_main() {
    ...
    function buildConfig(raw) {
        ...
        return {
            lang: raw.lang || normalizeLang(getCurrentLang()),
            mode: raw.mode || 'view',
    ...
    function EditFloatingButton(props) {
        ...
        return (
            <a href={props.href} className="fixed bottom-6 right-6 z-[60] inline-flex h-16 w-16 items-center justify-center rounded-full bg-fuchsia-600 text-lg font-bold text-white shadow-[0_0_0_10px_rgba(217,70,239,0.25)] transition hover:bg-fuchsia-500 focus:outline-none focus:ring-4 focus:ring-fuchsia-300">
                {props.label}
            </a>
        );
    }
    ...
    function IconButton(props) {
        return <button type="button" onClick={props.onClick} title={props.title} className={props.className}>{props.children}</button>;
    }
