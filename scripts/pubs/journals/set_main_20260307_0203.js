function set_main() {
    function getCurrentLang() {
        try {
            const i18n = window.StatKISS_I18N || {};
            const htmlLang = (document.documentElement.getAttribute('lang') || '').trim();
            const storedStatkissLang = (localStorage.getItem((i18n && i18n.LANG_KEY) || 'statkiss_lang') || '').trim();
            ...
            return winLang || storedStatkissLang || storedLang || htmlLang || browserLang || 'en';
        } catch (e) {
            return 'en';
        }
    }
    ...
    function normalizeLang(lang) {
        ...
        if (window.StatKISS_I18N && typeof window.StatKISS_I18N.resolveLangCode === 'function') {
            return window.StatKISS_I18N.resolveLangCode(raw);
        }
        ...
        if (value.indexOf('zh-') === 0) {
            if (value.indexOf('tw') >= 0 || value.indexOf('hk') >= 0 || value.indexOf('mo') >= 0 || value.indexOf('hant') >= 0) return 'zh-Hant';
            return 'zh-Hans';
        }
    ...
    function getTranslations(lang) {
        ...
        const current = dict[lang] || fallback;
