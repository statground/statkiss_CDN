function set_main() {
    ...
    function PublicPreview(props) {
        ...
        return (
            <div className={props.board.visible ? '' : 'opacity-70'}>
                {!props.board.visible ? <p className={props.isDark ? 'mb-4 text-sm text-amber-300' : 'mb-4 text-sm text-amber-700'}>{t(props.lang, 'hidden_notice')}</p> : null}
    ...
    function App() {
        ...
        React.useEffect(function () {
            async function loadInitial() {
                ...
                try {
                    ...
                    setMessage(t(langState, 'load_error'));
        ...
        async function reloadFromServer() {
            ...
            try {
                ...
                showMessage('error', t(langState, 'load_error'));
            } finally {
                setIsReloading(false);
            }
        }
        ...
        async function handleSave() {
            ...
            try {
                ...
                setHistory({ undo: [], redo: [] });
                setNote('');
                showMessage('success', t(langState, 'save_success'));
            } catch (error) {
                showMessage('error', t(langState, 'save_error'));
            } finally {
                setIsSaving(false);
            }
        }
        ...
        async function handleRestore(versionUuid) {
            ...
            try {
                ...
                showMessage('success', t(langState, 'restore_success'));
            } catch (error) {
                showMessage('error', t(langState, 'restore_error'));
            }
        }
        ...
        return (
            <div className={isDark ? 'mx-auto flex w-full max-w-screen-2xl flex-col gap-6 px-4 py-8 text-white' : 'mx-auto flex w-full max-w-screen-2xl flex-col gap-6 px-4 py-8 text-gray-900'}>
                ...
                <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)_360px]">
                    ...
                    <div className="flex flex-col gap-6">
                        <div className={panelClass}>
                            <h2 className={isDark ? 'text-lg font-semibold text-white' : 'text-lg font-semibold text-gray-900'}>{t(langState, 'save_note_label')}</h2>
                            <div className="mt-3">
                                <input type="text" className={inputClass} value={note} onChange={function (event) { setNote(event.target.value); }} placeholder={t(langState, 'note_placeholder')} />
                            </div>
                        </div>
                                        ...
                                        <div className={roleCardClass} key={version.version_uuid || version.uuid}>
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    ...
                                                    {version.note ? <div className={isDark ? 'mt-2 text-xs text-gray-300' : 'mt-2 text-xs text-gray-600'}>{version.note}</div> : null}
                                                </div>
