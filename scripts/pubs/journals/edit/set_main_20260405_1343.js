function set_main() {
    ...
    const BOARD_SYNC_STORAGE_KEY = 'statkiss:pubs:journals:editorial_board:sync';
    const BOARD_SYNC_EVENT_NAME = 'statkiss:journals:editorial_board:refresh';

    function broadcastBoardRefresh(payload) {
        const source = payload && typeof payload === 'object' ? payload : {};
        const detail = {
            version_uuid: String(source.current_version_uuid || source.saved_version_uuid || source.version_uuid || ''),
            current_version_uuid: String(source.current_version_uuid || source.saved_version_uuid || source.version_uuid || ''),
            saved_version_uuid: String(source.saved_version_uuid || source.current_version_uuid || source.version_uuid || ''),
            updated_at: String(source.updated_at || ''),
            timestamp: Date.now(),
        };
        try {
            localStorage.setItem(BOARD_SYNC_STORAGE_KEY, JSON.stringify(detail));
        } catch (e) {
            // Ignore storage write errors.
        }
        try {
            window.dispatchEvent(new CustomEvent(BOARD_SYNC_EVENT_NAME, { detail: detail }));
        } catch (e) {
    ...
    function resolveResponseMessage(payload, fallbackMessage) {
        const text = payload && typeof payload.message === 'string' ? payload.message.trim() : '';
        return text || fallbackMessage;
    }

    async function requireJsonOk(response, fallbackMessage) {
        const payload = await parseJsonResponse(response);
        if (!response.ok || payload.ok === false) {
            const error = new Error(resolveResponseMessage(payload, fallbackMessage));
    ...
    function localizeVersionAction(lang, action) {
        const rawAction = String(action || '').trim().toLowerCase();
        if (rawAction === 'save') return t(lang, 'action_save');
        if (rawAction === 'restore') return t(lang, 'action_restore');
        if (rawAction === 'seed') return t(lang, 'action_seed');
        return rawAction || t(lang, 'action_unknown');
    }

    function getDefaultRoleName(lang, index) {
        const suffix = Number(index || 0) > 0 ? ' ' + index : '';
        return t(lang, 'new_role') + suffix;
    }
    ...
    function createUuidLike() {
        try {
            ...
            // Fall back to pseudo-random UUID-like value below.
        }
    ...
    function normalizeBoardPayload(payload, options) {
        const opts = options || {};
        const preserveEmptyItems = !!opts.preserveEmptyItems;
        const source = payload && typeof payload === 'object' ? payload : {};
        ...
        const sections = rawSections.map(function (section, sectionIndex) {
            ...
            const normalizedItems = items.map(function (item, itemIndex) {
                return {
            ...
            return {
                ...
                items: (preserveEmptyItems ? normalizedItems : normalizedItems.filter(function (item) {
    ...
    function snapshotBoardState(value) {
        const normalized = normalizeBoardPayload(value, { preserveEmptyItems: true });
        return {
            visible: normalized.visible ? 1 : 0,
            sections: normalized.sections.map(function (section, sectionIndex) {
                return {
                    role_uuid: section.role_uuid || createUuidLike(),
                    role_key: section.role_key || slugifyRoleKey(section.role_name || ('role_' + (sectionIndex + 1))),
                    role_name: section.role_name || '',
                    role_order: sectionIndex + 1,
                    items: section.items.map(function (item, itemIndex) {
                        return {
                            item_uuid: item.item_uuid || createUuidLike(),
                            name: item.name || '',
    ...
    function buildSavePayload(value) {
        const normalized = normalizeBoardPayload(value);
        return {
            visible: normalized.visible ? 1 : 0,
            sections: normalized.sections.map(function (section, sectionIndex) {
                return {
        ...
        return (
            <li className="flex items-start list-none">
                ...
                <div className="flex flex-col">
                    ...
                    {props.affiliation ? <span className={props.isDark ? 'text-xs text-gray-200' : 'text-xs text-gray-500'}>({props.affiliation})</span> : null}
                </div>
            </li>
        );
    }
    ...
    function PublicPreview(props) {
        const totalItems = props.board.sections.reduce(function (sum, section) { return sum + section.items.length; }, 0);
        ...
        return (
            <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:grid-cols-5">
                    {props.board.sections.map(function (section) {
                        return (
    ...
    function App() {
        ...
        React.useEffect(function () {
            async function loadInitial() {
                ...
                try {
                    ...
                    const rawPayload = await requireJsonOk(response, t(langState, 'load_error'));
                    const payload = normalizeBoardPayload(rawPayload);
                    setBoard(snapshotBoardState(payload));
                    setVersions(payload.versions || []);
                    setSelectedRoleIndex(payload.sections.length ? 0 : -1);
                    setHistory({ undo: [], redo: [] });
                } catch (error) {
                    setMessage(error && error.message ? error.message : t(langState, 'load_error'));
        ...
        function handleAddRole() {
            ...
            mutateBoard(function (draft) {
                const nextRoleIndex = draft.sections.length + 1;
                const nextRoleName = getDefaultRoleName(langState, nextRoleIndex);
                draft.sections.push({
                    role_uuid: createUuidLike(),
                    role_key: 'editorial_role_' + nextRoleIndex,
                    role_name: nextRoleName,
                    role_order: nextRoleIndex,
                    items: []
                });
            }, nextIndex);
        }
        ...
        async function reloadFromServer() {
            ...
            try {
                ...
                showMessage('error', error && error.message ? error.message : t(langState, 'load_error'));
            } finally {
                setIsReloading(false);
            }
        }
        ...
        async function handleSave() {
            setIsSaving(true);
            try {
                const payload = buildSavePayload(board);
                ...
                const rawResult = await requireJsonOk(response, t(langState, 'save_error'));
                const result = normalizeBoardPayload(rawResult);
                setBoard(snapshotBoardState(result));
                setVersions(result.versions || []);
                setHistory({ undo: [], redo: [] });
                broadcastBoardRefresh(rawResult);
                showMessage('success', resolveResponseMessage(rawResult, t(langState, 'save_success')));
            } catch (error) {
                showMessage('error', error && error.message ? error.message : t(langState, 'save_error'));
            } finally {
                setIsSaving(false);
            }
        }
        ...
        async function handleRestore(versionUuid) {
            ...
            try {
                ...
                const rawResult = await requireJsonOk(response, t(langState, 'restore_error'));
                const result = normalizeBoardPayload(rawResult);
                setBoard(snapshotBoardState(result));
                setVersions(result.versions || []);
                setHistory({ undo: [], redo: [] });
                ...
                broadcastBoardRefresh(rawResult);
                showMessage('success', resolveResponseMessage(rawResult, t(langState, 'restore_success')));
            } catch (error) {
                showMessage('error', error && error.message ? error.message : t(langState, 'restore_error'));
            }
        }
                        ...
                        <div className={panelClass}>
                            ...
                            <div className="mt-4 rounded-2xl border border-dashed border-fuchsia-300/60 p-4">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        <p className={isDark ? 'text-sm font-semibold text-white' : 'text-sm font-semibold text-gray-900'}>{t(langState, 'visibility_label')}</p>
                                    ...
                                    <label className="inline-flex items-center gap-3 text-sm font-medium">
                                        <span>{board.visible ? t(langState, 'hide_board') : t(langState, 'show_board')}</span>
                                        ...
                                        <div className={roleCardClass} key={version.version_uuid || version.uuid}>
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    ...
                                                    <div className={isDark ? 'mt-1 text-xs text-gray-300' : 'mt-1 text-xs text-gray-600'}>{t(langState, 'action')}: {localizeVersionAction(langState, version.action)} · {t(langState, 'role_count')}: {version.role_count || 0} · {t(langState, 'member_count')}: {version.item_count || 0}</div>
