async function get_div_add_newsletter() {
	...
	if (gv_username != "" && (gv_role == "Administrator" || gv_role == "Officer")) {
		...
		for (var i = 0 ; i < Object.keys(data_newsletter).length ; i++) {
			document.getElementById("btn_delete_" + data_newsletter[Object.keys(data_newsletter)[i]].uuid).className = "flex flex-row justify-end items-end w-full cursor-pointer"
		}
	}
}