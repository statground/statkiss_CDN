async function get_list_newsletter() {
	// 클립보드에 파일 주소 복사하기
	function click_btn_copy_clipboard(url_file) {
		navigator.clipboard.writeText('https://' + window.location.host + '/' + url_file);
		alert("The file URL was saved to the clipboard.");
	}
		...
		const newsletter_list = Object.keys(props.data).map((btn_data) =>  
			<div class="flex flex-col bg-white rounded shadow h-fit">
				...
				<div class="flex flex-row justify-between items-center w-full">
					<div id={"btn_copy_clipboard_" + props.data[btn_data].uuid}
						 class="hidden"
						 onClick={() => click_btn_copy_clipboard(props.data[btn_data].url_file)}>
						<span class="h-6 m-2 text-sm text-blue-500 cursor-pointer">Copy Link</span>
					</div>

					<div id={"btn_delete_" + props.data[btn_data].uuid} 
						 class="hidden" 
