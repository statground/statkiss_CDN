            ...
            return `
                <article class="group relative overflow-hidden rounded-[1.5rem] border p-4 transition duration-200 ${cardSurface}">
                    ...
                    <div class="flex h-full flex-col gap-4 pl-1">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0 flex-1">
                                <span class="inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-semibold ${tone.ghost}">${escapeHtml(item.publish_date)}</span>
                                <h4 class="mt-3 text-lg font-black leading-snug tracking-tight ${dark ? 'text-white' : 'text-slate-950'}">${escapeHtml(headline)}</h4>
                            </div>
                            ${state.canManage ? `
                                <button type="button" data-action="delete-newsletter" data-uuid="${escapeHtml(item.uuid)}" class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border transition ${deleteButtonClass}" aria-label="${escapeHtml(t('newsletter.delete'))}" title="${escapeHtml(t('newsletter.delete'))}">
