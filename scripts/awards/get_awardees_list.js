async function get_awardees_list() {
	...
	function Div_awardees_list(props) {
		function Div_sub(props) {
			...
			const awardeesList = Object.keys(props.data).map((btn_data) =>
				<Div_person name={props.data[btn_data].name} affiliation={props.data[btn_data].affiliation} award_url_icon={props.data[btn_data].award_url_icon} />
			)
	...
	const data = await fetch("/awards/ajax_get_awardees_list/?tag=" + url)
	...
	let data_res = {}
	let data_keys = Array.from(new Set(Object.values(data).map(function(element){ return element.year })))

	for (var i = 0 ; i < data_keys.length ; i++) {
		data_res[data_keys[i]] = Object.values(data).filter(it => it.year == data_keys[i])
	}
