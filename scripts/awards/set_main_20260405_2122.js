...
let awards_page_config_cache = null;

function get_awards_page_config() {
    if (awards_page_config_cache) {
        return awards_page_config_cache;
    }

    try {
        const script = document.getElementById("statkiss-awards-page-config");
        awards_page_config_cache = script ? JSON.parse(script.textContent || "{}") : {};
    } catch (error) {
        awards_page_config_cache = {};
    }

    return awards_page_config_cache;
}

function get_awards_mode() {
    const config = get_awards_page_config();
...
function is_awards_edit_mode() {
    return get_awards_mode() === "edit";
}

function can_manage_current_awards() {
    const config = get_awards_page_config();
    return !!(config && config.can_manage_awards);
}

function get_awards_public_path() {
    const config = get_awards_page_config();
    if (config && typeof config.public_path === "string" && config.public_path) {
        return config.public_path;
    }

    return build_awards_path(`/awards/${encodeURIComponent(url || "")}/`);
}

function get_awards_edit_path() {
    const config = get_awards_page_config();
    if (config && typeof config.edit_path === "string" && config.edit_path) {
        return config.edit_path;
    }

    return build_awards_path(`/awards/${encodeURIComponent(url || "")}/edit/`);
}
...
function awards_surface_class(lightClass, darkClass) {
    return is_awards_dark_mode() ? darkClass : lightClass;
}
...
function Div_btn_submit() {
    return (
        <button
            ...
            className="py-1.5 px-5 text-white bg-blue-700 font-medium rounded-lg text-sm w-full md:w-auto text-center hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-400 dark:focus:ring-blue-950"
        >
...
function Div_btn_submit_loading() {
    return (
        <button
            type="button"
            className="py-1.5 px-5 text-white bg-blue-700 font-medium rounded-lg text-sm w-full md:w-auto text-center cursor-not-allowed hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-400 dark:focus:ring-blue-950"
        >
...
function Div_add_awardees() {
    return (
        <div>
            <div className={`flex flex-col w-full justify-end items-center space-x-4 mb-8 px-8 py-6 shadow rounded-2xl border space-y-4 md:px-5 ${awards_surface_class("border-blue-300 bg-white", "border-slate-600 bg-slate-900")}`}>
                <div className="flex-row justify-start items-start w-full">
                    <p className={`font-bold ${awards_text_class("text-gray-900", "text-white")}`}>{awards_t("awards.manage.add_awardee")}</p>
                </div>

                <div className="flex flex-col w-full space-y-4">
                    <div className="grid grid-cols-2 w-full gap-8 md:grid-cols-1 md:gap-4">
                        <div className="flex flex-row justify-center items-center space-x-4 w-full md:flex-col md:items-start md:space-x-0 md:space-y-2">
                            <p className={`min-w-24 ${awards_text_class("text-gray-900", "text-gray-100")}`}>{awards_t("awards.form.year")}</p>
                            <input
                                type="number"
                                id="txt_year"
                                min="1000"
                                ...
                                className={`border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ${awards_surface_class("bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-500", "bg-slate-800 border-slate-600 text-white placeholder-slate-400")}`}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full md:grid-cols-1">
                        <div className="flex flex-row justify-center items-center space-x-4 w-full md:flex-col md:items-start md:space-x-0 md:space-y-2">
                            <p className={`min-w-24 ${awards_text_class("text-gray-900", "text-gray-100")}`}>{awards_t("awards.form.name")}</p>
                            <input
                                type="text"
                        ...
                        <div className="flex flex-row justify-center items-center space-x-4 w-full md:flex-col md:items-start md:space-x-0 md:space-y-2">
                            <p className={`min-w-24 ${awards_text_class("text-gray-900", "text-gray-100")}`}>{awards_t("awards.form.affiliation")}</p>
                            <input
                                type="text"
                ...
                <div className="flex flex-row justify-end items-center w-full">
                    <div className="flex flex-row justify-end items-center" id="div_btn_submit">
                        {toggle_btn_submit ? <Div_btn_submit_loading /> : <Div_btn_submit />}
                    </div>
                </div>
            </div>
        </div>
...
function render_awardees_list(groupedData = {}) {
    const showDeleteButtons = is_awards_edit_mode() && can_manage_current_awards();

    function Div_person(props) {
        return (
            <li className="flex flex-row items-start">
                <img className="w-3.5 h-3.5 mr-2 mt-1" src={props.award_url_icon} />
                ...
                {showDeleteButtons ? (
                    <button
                        type="button"
                        className="flex flex-row justify-end items-end rounded-md p-1 hover:bg-red-100 dark:hover:bg-red-950"
                        onClick={() => click_btn_delete(props.uuid)}
                    >
                        <img
                            className="object-scale-down h-4 m-1 cursor-pointer"
    ...
    function Div_awardees_list(props) {
        const yearKeys = Object.keys(props.data).sort((a, b) => Number(b) - Number(a));

        if (!yearKeys.length) {
            return <div className={`border-l min-h-8 ${awards_text_class("border-gray-200", "border-gray-100")}`}></div>;
        }

        const subList = yearKeys.map((yearKey) => (
            <Div_sub key={yearKey} year={yearKey} items={props.data[yearKey]} />
        ));

        return <ol className={`grid grid-cols-4 border-l md:grid-cols-2 sm:grid-cols-1 ${awards_text_class("border-gray-200", "border-gray-100")}`}>{subList}</ol>;
    }
...
function get_div_add_awardee() {
    ...
    const canEditHere = is_awards_edit_mode() && can_manage_current_awards();
    if (!canEditHere) {
    ...
    render_submit_button(toggle_btn_submit);
}
        ...
        return (
            <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
                ...
                <div className="w-full" id="div_awardees_list">
                    <ol className={`grid grid-cols-4 border-l animate-pulse md:grid-cols-2 sm:grid-cols-1 ${awards_text_class("border-gray-200", "border-gray-100")}`}>
                        <Div_skeleton_list />
                        <Div_skeleton_list />
                        <Div_skeleton_list />
                        <Div_skeleton_list />
