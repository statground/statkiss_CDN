function set_main() {
  ...
  (function injectConstitutionThemeFallback(){
    ...
    style.textContent = `
      ...
      html.dark #div_main .cst-bg { background: transparent; }
      html.dark #div_main .cst-title { color: #ffffff; }
      html.dark #div_main .cst-sub { color: #94a3b8; }
      html.dark #div_main .cst-h { color: #ffffff; }
      html.dark #div_main .cst-p { color: #e2e8f0; }
      html.dark #div_main .cst-hr { border-color: rgba(51,65,85,0.75); }
      html.dark #div_main .cst-li { color: #e2e8f0; }

      body.dark #div_main .cst-bg { background: transparent; }
      body.dark #div_main .cst-title { color: #ffffff; }
      body.dark #div_main .cst-sub { color: #94a3b8; }
      body.dark #div_main .cst-h { color: #ffffff; }
      body.dark #div_main .cst-p { color: #e2e8f0; }
      body.dark #div_main .cst-hr { border-color: rgba(51,65,85,0.75); }
      body.dark #div_main .cst-li { color: #e2e8f0; }
    `;
  ...
  function Div_main() {
    ...
    // If the i18n bundle loads after set_main, bump a state once it becomes available.
    useEffect(() => {
    ...
    // Language sync: header events + localStorage + <html lang="..."> mutations
    useEffect(() => {
      const applyLang = (raw) => {
    ...
    function Article({ titleKey, paragraphs, children }) {
      return (
        <div className="mb-10 px-4">
          <h3 className="cst-h mb-4 text-lg font-medium">
            {t(titleKey)}
          </h3>
          <div className="space-y-4">
            {paragraphs && paragraphs.map((k, idx) => (
              <p key={idx} className="cst-p text-md">
                {t(k)}
              </p>
            ))}
            {children}
    ...
    return (
      <section className="cst-bg">
        <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
          <div className="mx-auto max-w-screen-lg text-center">
            <h2 className="cst-title mb-2 text-4xl tracking-tight font-extrabold">
              {t('constitution.title')}
            </h2>
            <p className="cst-sub mb-8 lg:text-lg">
              {t('constitution.subtitle')}
            </p>
          </div>

          <div className="flex flex-col pt-8 text-left border-t cst-hr">
            ...
            <Article titleKey="constitution.article2.title" paragraphs={["constitution.article2.p1"]}>
              <ul className="space-y-1 list-disc list-inside">
                {bullets.map((b, i) => (
                  <li key={i} className="cst-li">
                    {b}
                  </li>
                ))}
              </ul>
              <p className="cst-p text-md">
