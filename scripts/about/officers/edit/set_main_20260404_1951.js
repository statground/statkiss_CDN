(function () {
  ...
  function getSupportedLangCodes() {
    const helper = getGlobalI18N();
    if (helper && Array.isArray(helper.languages)) return helper.languages.map((item) => item.code);
  ...
  function set_main() {
    ...
    function Badge({ tone = 'slate', children }) {
      ...
      return (
        <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${toneMap[tone] || toneMap.slate}`}>
          {children}
        </span>
      );
    }
    ...
    function IconButton({ title, onClick, tone = 'slate', disabled = false, children }) {
      ...
      return (
        <button
          ...
          disabled={disabled}
          onClick={onClick}
          className={`inline-flex h-10 w-10 items-center justify-center rounded-2xl border text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-45 ${toneMap[tone] || toneMap.slate}`}
        >
          {children}
        </button>
      );
    ...
    function TextActionButton({ text, onClick, tone = 'slate', disabled = false }) {
      ...
      return (
        <button
          ...
          disabled={disabled}
          onClick={onClick}
          className={`inline-flex items-center rounded-2xl border px-4 py-2 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-45 ${toneMap[tone] || toneMap.slate}`}
        >
          {text}
        </button>
      );
    ...
    function EditorPage() {
      ...
      function applyServerPayload(payload, successMessage) {
        const nextTerms = normalizeTerms(payload && payload.terms);
        setTerms(nextTerms);
        setVersions(normalizeVersions(payload && payload.versions));
        setVersioningEnabled(Boolean(payload && payload.versioning_enabled));
        setSelectedTermId((previous) => pickPreferredTermId(nextTerms, previous || selectedTermId, showHiddenTerms));
      ...
      async function load() {
        ...
        try {
          ...
          setVersions(normalizeVersions(payload && payload.versions));
          setVersioningEnabled(Boolean(payload && payload.versioning_enabled));
          setSelectedTermId(pickPreferredTermId(nextTerms, '', false));
      ...
      function commitMutation(mutator) {
        const before = normalizeTerms(terms);
      ...
      function handleUndo() {
        ...
        historyRef.current.future.push(deepClone(normalizeTerms(terms)));
        setTerms(normalizeTerms(previous));
        setHistoryTick((value) => value + 1);
        setError('');
        setMessage('');
      }
      ...
      function handleRedo() {
        ...
        historyRef.current.past.push(deepClone(normalizeTerms(terms)));
        if (historyRef.current.past.length > HISTORY_LIMIT) historyRef.current.past.shift();
      ...
      function addTerm() {
        ...
        setSelectedTermId(newTerm._clientId);
      }
      ...
      return (
        <main className="min-h-screen bg-slate-50 py-8 dark:bg-slate-950" data-statkiss-officers-editor>
