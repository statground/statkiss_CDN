  ...
  function set_main() {
    ...
    function EditorPage() {
      ...
      function applyServerPayload(payload, successMessage) {
        ...
        const selectedBefore = selectedTerm ? (selectedTerm.uuid || selectedTerm._clientId || '') : '';
        const hiddenSelected = nextTerms.some((term) => {
          const termIdentity = term.uuid || term._clientId || '';
          return termIdentity && termIdentity === selectedBefore && normalizeFlag(term.visible, 1) !== 1;
        });
        if (hiddenSelected) setShowHiddenTerms(true);
        setTerms(nextTerms);
        setVersions(normalizeVersions(payload && payload.versions));
        setVersioningEnabled(Boolean(payload && payload.versioning_enabled));
        setSelectedTermId((previous) => pickPreferredTermId(nextTerms, previous || selectedTermId || selectedBefore, hiddenSelected ? true : showHiddenTerms));
      ...
      async function handleSave() {
        ...
        setMessage('');
        try {
          const submittedTerms = normalizeTerms(terms);
          const selectedBefore = selectedTerm ? (selectedTerm.uuid || selectedTerm._clientId || '') : '';
          const hiddenSelected = submittedTerms.some((term) => {
            const termIdentity = term.uuid || term._clientId || '';
            return termIdentity && termIdentity === selectedBefore && normalizeFlag(term.visible, 1) !== 1;
          });
          if (hiddenSelected) setShowHiddenTerms(true);
          const payload = await saveEditorPayload(submittedTerms);
