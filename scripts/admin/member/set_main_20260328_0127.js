...
function member_management_get_palette(themeMode) {
    if (themeMode === 'dark') {
        return {
            page: 'bg-transparent',
            navWrap: 'rounded-2xl border border-slate-800 bg-slate-950/90 shadow-[0_20px_45px_rgba(2,6,23,0.45)] p-2',
            navActive: 'bg-slate-800 border border-blue-400/35 text-blue-200 shadow-[0_10px_30px_rgba(37,99,235,0.18)]',
            navInactive: 'bg-transparent border border-transparent text-slate-400 hover:bg-slate-900 hover:border-slate-800 hover:text-slate-200',
            panel: 'rounded-2xl border border-slate-800 bg-slate-950/88 shadow-[0_18px_40px_rgba(2,6,23,0.42)] p-6',
            panelSoft: 'rounded-2xl border border-slate-800 bg-slate-900/72 p-4',
            heading: 'text-slate-50',
            body: 'text-slate-200',
            muted: 'text-slate-400',
            input: 'w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20',
            checkboxItem: 'flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900/85 px-4 py-3 transition hover:border-slate-700 hover:bg-slate-900',
            checkbox: 'h-4 w-4 rounded border-slate-600 bg-slate-950 text-blue-500 focus:ring-blue-500',
            ...
            secondaryBtn: 'inline-flex items-center justify-center rounded-xl border border-slate-700 bg-slate-900 px-5 py-3 text-sm font-semibold text-slate-100 transition hover:bg-slate-800 focus:outline-none focus:ring-4 focus:ring-slate-400/20',
            tertiaryBtn: 'inline-flex items-center justify-center rounded-xl border border-slate-700 bg-slate-950 px-5 py-3 text-sm font-semibold text-slate-100 transition hover:bg-slate-900 focus:outline-none focus:ring-4 focus:ring-slate-400/20',
            countNumber: 'text-slate-50',
            countLabel: 'text-slate-300',
            memberCard: 'rounded-2xl border border-slate-800 bg-slate-950/88 shadow-[0_18px_40px_rgba(2,6,23,0.42)] p-5',
            sectionCard: 'rounded-2xl border border-slate-800 bg-slate-900/72 p-4',
            sectionLabel: 'text-sm font-semibold text-slate-200',
            name: 'text-lg font-semibold text-slate-50',
            email: 'text-sm text-slate-400',
            pendingBadge: 'inline-flex items-center rounded-full border border-rose-400/30 bg-rose-500/12 px-3 py-1 text-xs font-semibold text-rose-200',
            officerBadge: 'inline-flex items-center rounded-full border border-amber-400/30 bg-amber-500/12 px-3 py-1 text-xs font-semibold text-amber-100',
            metaChip: 'inline-flex items-center rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs text-slate-200',
            emptyWrap: 'rounded-2xl border border-dashed border-slate-800 bg-slate-950/70 px-6 py-12 text-center text-slate-300',
            emptyMuted: 'text-slate-500',
            link: 'text-xs break-all text-blue-300 underline underline-offset-2',
            dangerText: 'text-rose-300',
            loadingCard: 'rounded-2xl border border-slate-800 bg-slate-950/88',
            skeletonStrong: 'bg-slate-800',
            skeletonSoft: 'bg-slate-900',
            skeletonMuted: 'bg-slate-700/80',
            spinnerTrack: '#1e293b',
    ...
    return {
        ...
        skeletonStrong: 'bg-slate-300',
        skeletonSoft: 'bg-slate-200',
        skeletonMuted: 'bg-slate-300/80',
        spinnerTrack: '#E5E7EB',
        spinnerFill: 'currentColor'
    }
}
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div class="space-y-2">
                                ...
                                <div class={'h-3 w-56 rounded-full ' + palette.skeletonSoft}></div>
                            </div>
                            <div class={'h-10 w-40 rounded-xl ' + palette.skeletonSoft}></div>
                        </div>
                        <div class="grid grid-cols-1 gap-4 xl:grid-cols-5">
                            <div class={'xl:col-span-3 rounded-2xl p-4 ' + palette.panelSoft}>
                                <div class={'h-4 w-32 rounded-full mb-4 ' + palette.skeletonStrong}></div>
                                <div class="grid grid-cols-2 gap-2 lg:grid-cols-3">
                                    {[0, 1, 2, 3, 4, 5].map(chip => <div key={chip} class={'h-9 rounded-full ' + palette.skeletonMuted}></div>)}
                                </div>
                            </div>
                            <div class={'xl:col-span-2 rounded-2xl p-4 ' + palette.panelSoft}>
                                <div class={'h-4 w-32 rounded-full mb-4 ' + palette.skeletonStrong}></div>
                                <div class={'h-9 w-40 rounded-full ' + palette.skeletonMuted}></div>
                            </div>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            {[0, 1, 2].map(meta => <div key={meta} class={'h-8 w-32 rounded-full ' + palette.skeletonSoft}></div>)}
                        </div>
                    </div>
                </div>
            ))}
    ...
    return (
        <div class={palette.panel + ' animate-pulse'}>
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-3">
                    <div class={'h-3 w-36 rounded-full ' + palette.skeletonSoft}></div>
                    <div class={'h-10 w-28 rounded-full ' + palette.skeletonStrong}></div>
                    <div class={'h-3 w-64 rounded-full ' + palette.skeletonSoft}></div>
                </div>
                <div class={'h-12 w-44 rounded-xl ' + palette.skeletonSoft}></div>
            </div>
        </div>
    )
}
...
async function click_btn_search() {
    ...
    if (target) {
        ReactDOM.render(<Div_btn_search_loading />, target)
    }
    ...
    try {
        ...
        if (target) {
            ReactDOM.render(<Div_btn_search />, target)
        }
    }
}
...
async function click_btn_like_change_password(email) {
    ...
    if (data && data.url) {
        ...
        anchor.className = member_management_get_palette(member_patch_get_theme_mode()).link
        ...
        return (
            <div class={'flex flex-col w-full space-y-6 ' + palette.page} dir={member_patch_is_rtl() ? 'rtl' : 'ltr'}>
                ...
                <div class="grid grid-cols-1 gap-6 xl:grid-cols-12 items-start">
                    <div class="xl:col-span-4 2xl:col-span-3 xl:sticky xl:top-6">
                        <div id="div_member_search_filter"></div>
                    </div>
