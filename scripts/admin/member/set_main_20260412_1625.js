...
;(function () {
    ...
    window.member_patch_toggle_bot_review_editor = function (uuidUser) {
        ...
        if (!next) delete BOT_PENDING_ACTION_STATE[key];
        botCleanupRerenderList();
        return next;
    };

    window.member_patch_select_bot_review_action = function (uuidUser, action) {
        const key = botCleanupSafeText(uuidUser).trim();
        const nextAction = action === 'mark_review' || action === 'mark_suspected' ? action : '';
        if (!key) return false;
        BOT_EDITOR_STATE[key] = true;
        if (nextAction) BOT_PENDING_ACTION_STATE[key] = nextAction;
        else delete BOT_PENDING_ACTION_STATE[key];
        botCleanupRerenderList();
        return true;
    };
    ...
    window.member_patch_apply_bot_review_action = function (uuidUser, email, action) {
        ...
        if (nextAction !== 'mark_review' && nextAction !== 'mark_suspected') return false;
        return window.member_patch_run_bot_review_action(key, email, nextAction, { skipConfirm: true });
    };

    window.member_patch_clear_bot_review_action = function (uuidUser, email) {
        const key = botCleanupSafeText(uuidUser).trim();
        delete BOT_PENDING_ACTION_STATE[key];
        return window.member_patch_run_bot_review_action(key, email, 'clear');
    };

    window.member_patch_save_bot_reason = function (uuidUser, email) {
        const key = botCleanupSafeText(uuidUser).trim();
        if (!key || typeof window.click_btn_change_member_bot_status !== 'function') {
            alert(admin_t('members.load_error'));
            return false;
        }
        return window.click_btn_change_member_bot_status(key, 'save_reason', email || '', botCleanupReadReason(key), {
            skipConfirm: true,
            afterSuccess: () => {
                botCleanupRerenderList();
            },
    ...
    function botCleanupRenderEditor(member, themeMode, palette, mode) {
        ...
        const showReasonForm = !isManagementMode || pendingAction === 'mark_review' || pendingAction === 'mark_suspected';
                ...
                <div class={palette.sectionCard}>
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                            <button
                                type="button"
                                class={memberPatchBotActionButtonClassV2(themeMode, 'warning')}
                                onClick={() => isManagementMode ? window.member_patch_select_bot_review_action(key, 'mark_review') : window.member_patch_run_bot_review_action(key, member.email, 'mark_review')}>
                                {admin_t('members.bot.action.mark_review')}
                            </button>
                            <button
                                type="button"
                                class={memberPatchBotActionButtonClassV2(themeMode, 'success')}
                                onClick={() => window.member_patch_clear_bot_review_action(key, member.email)}>
                                {admin_t('members.bot.action.clear')}
                            </button>
                            <button
                                type="button"
                                class={memberPatchBotActionButtonClassV2(themeMode, 'danger')}
                                onClick={() => isManagementMode ? window.member_patch_select_bot_review_action(key, 'mark_suspected') : window.member_patch_run_bot_review_action(key, member.email, 'mark_suspected')}>
                                {admin_t('members.bot.action.mark_suspected')}
                            </button>
                        </div>
                        ...
                        {showReasonForm ? (
                            <div class="space-y-2">
                                ...
                                <textarea
                                    key={isManagementMode ? ('bot_manage_reason_' + key + '_' + pendingAction) : ('bot_review_reason_' + key + '_' + botCleanupSafeText(member.bot_decision_reason))}
                                    id={botCleanupReasonInputId(key)}
                                    rows="4"
                                    defaultValue={isManagementMode ? '' : botCleanupSafeText(member.bot_decision_reason)}
                                    placeholder={reasonPlaceholder}
                                    class={palette.input + ' min-h-[112px] resize-y'}></textarea>
                                <div class="flex justify-end">
                                    <button
                                        type="button"
                                        class={botCleanupSmallPrimaryButtonClass(themeMode)}
                                        onClick={() => isManagementMode ? window.member_patch_apply_bot_review_action(key, member.email, pendingAction) : window.member_patch_save_bot_reason(key, member.email)}>
