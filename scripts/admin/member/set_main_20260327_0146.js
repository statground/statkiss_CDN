    ...
    const inputRow = (label, id) => (
        <div class="flex flex-row justify-center items-center px-4 w-full">
            ...
            <input type="text" id={id}
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-3/4" />
        </div>
    )
        ...
        <div>
            <div class="flex flex-col w-full justify-end items-center space-x-4 px-8 py-4 bg-blue-100 border border-blue-300 shadow space-y-4 rounded-lg md:px-2">
                <div class="flex-row justify-start items-start w-full">
                    <p class="font-bold">{admin_t('members.search.title')}</p>
                </div>
                ...
                <div class="flex flex-col justify-center items-center w-full space-y-1">
                    ...
                    <ul class="grid grid-cols-3 items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg md:grid-cols-2">
                        {checkboxItem('check_member_pending_student', admin_role_label('Student Member'))}
                    </ul>
                </div>
...
async function click_btn_download_list() {
	...
	let url = admin_build_url('/admin/ajax_download_member_list/') + '?txt_temp=blank'
	...
	url += "&check_member_addon_none=" + check_member_addon_none
	url += "&check_member_addon_kssjoint=" + check_member_addon_kssjoint
...
async function click_btn_pending_student(email) {
    ...
    await fetch(admin_build_url('/admin/ajax_change_pending_student/'), {
        method: 'post',
        headers: { 'X-CSRFToken': getCookie('csrftoken') },
        body: request_data
    })
...
async function get_counter_graph(type) {
    let divGraphMonthly = document.getElementById('graph_monthly')
    let divGraphYearly = document.getElementById('graph_yearly')
    let classDivGraph = 'p-4 rounded-lg bg-gray-50 w-full'
    ...
    const data = await fetch(admin_build_url('/admin/ajax_get_member_table/'))
        .then(res => res.json())
        .then(res => res)
    ...
    if (type == 'monthly') {
        ...
        options['title'] = admin_t('members.graph.monthly_signups')
        ...
        options['title'] = admin_t('members.graph.yearly_signups')
        bar(data.yearly, 'graph_yearly', options)
    }
}

async function get_member_summary() {
    function getRoleCount(list, role) {
        const found = Object.values(list).filter(it => it.role == role)
        ...
        return (
            <div class="flex flex-col justify-center items-center w-full space-y-4">
                <div class="flex flex-col justify-center items-center w-full border border-blue-200 shadow p-8 space-y-4 bg-white rounded-lg">
                    <h2 class="mb-2 text-3xl font-extrabold text-gray-900 md:text-xl text-center">
                        <span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">
                            {admin_t('members.summary.membership')}
                        </span>
                    </h2>

                    <div class="grid grid-cols-4 md:grid-cols-2 gap-4 w-full">
                        <Div_card title={admin_t('admin.roles.admin')} count={getRoleCount(props.data.membership, 'Administrator') + getRoleCount(props.data.membership, 'Developer')} />
                        <Div_card title={admin_role_label('Lifetime Member')} count={getRoleCount(props.data.membership, 'Lifetime Member')} />
                        <Div_card title={admin_role_label('Regular Member')} count={getRoleCount(props.data.membership, 'Regular Member')} />
                        <Div_card title={admin_role_label('Spouse Member')} count={getRoleCount(props.data.membership, 'Spouse Member')} />
                        <Div_card title={admin_role_label('Student Member')} count={getRoleCount(props.data.membership, 'Student Member')} />
                        <Div_card title={admin_role_label('Non-member')} count={getRoleCount(props.data.membership, 'Non-member')} />
                    </div>
                </div>
                ...
                <div class="flex flex-col justify-center items-center w-full border border-blue-200 shadow p-8 space-y-4 bg-white rounded-lg">
                    <h2 class="mb-2 text-3xl font-extrabold text-gray-900 md:text-xl text-center">
                        <span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">
                            {admin_t('members.summary.signed_up')}
                        </span>
                    </h2>

                    <div class="grid grid-cols-4 md:grid-cols-2 gap-4 w-full">
                        <Div_card title={admin_t('admin.common.total')} count={props.data.member_joined.cnt_total} />
                        <Div_card title={admin_t('admin.common.current_year')} count={props.data.member_joined.cnt_yearly} />
                        <Div_card title={admin_t('admin.common.current_month')} count={props.data.member_joined.cnt_monthly} />
                        <Div_card title={admin_t('admin.common.today')} count={props.data.member_joined.cnt_daily} />
                    </div>
                </div>
                ...
                <div id="div_graph" class="w-full bg-white border border-gray-200 rounded-lg shadow p-6">
                    <div class="flex flex-row justify-start items-center space-x-8 py-2 mb-4 border-b border-gray-900">
                        <span class="hover:underline cursor-pointer" onClick={() => get_counter_graph('monthly')}>{admin_t('admin.common.monthly')}</span>
                        <span class="hover:underline cursor-pointer" onClick={() => get_counter_graph('yearly')}>{admin_t('admin.common.yearly')}</span>
                    </div>
    ...
    const data = await fetch(admin_build_url('/admin/ajax_get_member_counter/'))
        .then(res => res.json())
        .then(res => res)
...
async function get_member_list(mode) {
	...
	const data = await fetch(admin_build_url("/admin/ajax_get_member_list/"), {
						method: "post", 
						headers: { "X-CSRFToken": getCookie("csrftoken"), },
						body: request_data
						})
...
function member_patch_prepare_membership_chart_data(dashboard, period) {
    ...
    const labelsRaw = Array.from(new Set(rows.map(row => String((row || {}).date || '')).filter(Boolean))).sort()
    const grouped = {}
...
function member_patch_prepare_signup_chart_data(dashboard, period) {
    ...
    return {
        labels: sorted.map(row => member_patch_format_period_label(row.date, period)),
        signups: sorted.map(row => member_patch_safe_number(row.signup_count)),
        cumulative: sorted.map(row => member_patch_safe_number(row.cumulative_signup_count))
    }
}
...
function member_patch_build_membership_chart_option(dashboard, period, themeMode) {
    ...
    const chartData = member_patch_prepare_membership_chart_data(dashboard, period)
    ...
    return {
        ...
        color: ['#4f6bdc', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#64748b'],
        animationDuration: 300,
        title: {
            text: admin_t('members.chart.membership_title'),
            left: 'center',
            top: 18,
            textStyle: {
        ...
        toolbox: {
            ...
            feature: {
                ...
                saveAsImage: {
                    title: 'Download',
                    name: 'statkiss_membership_' + period
                }
            }
        },
        grid: {
...
function Div_member_dashboard_loading(props) {
    ...
    return (
        <div class="flex flex-col w-full space-y-6 animate-pulse">
            ...
            <div class={'w-full rounded-xl border h-[920px] ' + palette.loadingCard}></div>
