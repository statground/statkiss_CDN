...
;(function () {
    ...
    async function click_btn_change_member_bot_status_v2(uuid_user, action, email, reason, options) {
        ...
        try {
            ...
            if (action !== 'save_reason' && typeof get_member_summary === 'function') {
                setTimeout(() => {
...
;(function () {
    ...
    window.member_patch_run_bot_review_action = function (uuidUser, email, action) {
        const key = botCleanupSafeText(uuidUser).trim();
        if (!key || typeof window.click_btn_change_member_bot_status !== 'function') {
            alert(admin_t('members.load_error'));
            return false;
        }
        const reason = action === 'clear' ? '' : memberPatchReadBotReason(key);
    ...
    window.member_patch_apply_bot_review_action = function (uuidUser, email, action) {
        return window.member_patch_run_bot_review_action(uuidUser, email, action || 'mark_review');
    };

    window.member_patch_clear_bot_review_action = function (uuidUser, email) {
        return window.member_patch_run_bot_review_action(uuidUser, email, 'clear');
    };

    const BOT_REASON_AUTOSAVE_TIMERS = window.member_patch_bot_reason_autosave_timers_final || {};
    window.member_patch_bot_reason_autosave_timers_final = BOT_REASON_AUTOSAVE_TIMERS;

    function botCleanupReasonSavedTarget(uuidUser) {
        return document.getElementById('txt_member_bot_reason_saved_' + botCleanupSafeText(uuidUser).trim());
    }

    function botCleanupSetReasonSaved(uuidUser, visible) {
        const target = botCleanupReasonSavedTarget(uuidUser);
        if (!target) return;
        if (visible) {
            target.textContent = admin_t('members.bot.form.reason_saved');
    ...
    window.member_patch_queue_bot_reason_autosave = function (uuidUser, email) {
        const key = botCleanupSafeText(uuidUser).trim();
        if (!key || typeof window.click_btn_change_member_bot_status !== 'function') return false;
        if (BOT_REASON_AUTOSAVE_TIMERS[key]) {
            clearTimeout(BOT_REASON_AUTOSAVE_TIMERS[key]);
        }
        botCleanupSetReasonSaved(key, false);
        BOT_REASON_AUTOSAVE_TIMERS[key] = setTimeout(() => {
            window.click_btn_change_member_bot_status(key, 'save_reason', email || '', memberPatchReadBotReason(key), {
                skipConfirm: true,
                silent: true,
                afterSuccess: () => {
                    botCleanupSetReasonSaved(key, true);
                    setTimeout(() => botCleanupSetReasonSaved(key, false), 2600);
                },
            }).catch((error) => {
                console.error(error);
            }).finally(() => {
                delete BOT_REASON_AUTOSAVE_TIMERS[key];
            });
        }, 3000);
        return true;
    };
    ...
    window.member_patch_apply_bot_review_form = function (uuidUser, email) {
        return window.member_patch_run_bot_review_action(uuidUser, email, 'mark_review');
    };
    ...
    window.member_patch_resend_bot_verification = function (uuidUser, email) {
        ...
        return window.click_btn_change_member_bot_status(key, 'resend_verification', email || '', memberPatchReadBotReason(key));
    };
    ...
    function botCleanupRenderEditor(member, themeMode, palette) {
        ...
        const messageClass = themeMode === 'dark' ? 'text-xs text-emerald-200' : 'text-xs text-emerald-700';
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                                ...
                                <button type="button" class={memberPatchBotActionButtonClassV2(themeMode, 'warning')} onClick={() => window.member_patch_run_bot_review_action(key, member.email, 'mark_review')}>
                                    {admin_t('members.bot.action.mark_review')}
                                </button>
                                <button type="button" class={memberPatchBotActionButtonClassV2(themeMode, 'danger')} onClick={() => window.member_patch_run_bot_review_action(key, member.email, 'mark_suspected')}>
                                    {admin_t('members.bot.action.mark_suspected')}
                                </button>
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea
                                    ...
                                    onInput={() => window.member_patch_queue_bot_reason_autosave(key, member.email)}
                                    class={palette.input + ' min-h-[112px] resize-y'}></textarea>
                                <p id={'txt_member_bot_reason_saved_' + key} class={messageClass + ' hidden'}></p>
                            </div>
                        </div>
                    </div>
                </div>
