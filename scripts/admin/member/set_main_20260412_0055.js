    ...
    function member_patch_fix_get_filter_state() {
        if (typeof member_patch_clone_filter_state === 'function') {
            if (typeof gv_member_filter_state !== 'undefined' && gv_member_filter_state) {
                return member_patch_clone_filter_state(gv_member_filter_state)
            }
            if (typeof member_patch_read_member_filter_state_from_dom === 'function') {
                return member_patch_clone_filter_state(member_patch_read_member_filter_state_from_dom())
            }
        }
        return null
    }
...
;(function () {
    ...
    async function click_btn_change_member_bot_status_v2(uuid_user, action, email, reason) {
        if (!uuid_user || !action) return
        const confirmKeyMap = {
            clear: 'members.bot.confirm.clear',
            mark_review: 'members.bot.confirm.mark_review',
        ...
        try {
            ...
            requestData.append('action', action)
            requestData.append('email', email || '')
            requestData.append('reason', String(reason || '').trim())
            const response = await member_patch_fetch_json(member_patch_build_url('/admin/ajax_change_member_bot_status/'), {
                method: 'post',
                headers: { 'X-CSRFToken': getCookie('csrftoken') },
                body: requestData,
...
;(function () {
    ...
    function botCleanupStatusBucket(member) {
        ...
        if (/버림|discard/i.test(decision)) return 'discarded';
    ...
    function botCleanupStatusLabel(member) {
        ...
        if (bucket === 'discarded') return admin_t('members.bot.badge.discarded');
        if (bucket === 'review') return admin_t('members.bot.status.review_queue');
        if (bucket === 'suspected') return admin_t('members.bot.status.suspected');
    ...
    window.member_patch_toggle_bot_review_editor = function (uuidUser) {
        ...
        const button = document.getElementById('btn_member_toggle_bot_editor_' + key);
        if (button) {
            button.textContent = admin_t(next ? 'members.bot.toggle.close' : 'members.bot.toggle.open');
            button.setAttribute('aria-expanded', next ? 'true' : 'false');
        }
        return next;
    };

    window.member_patch_apply_bot_review_action = function (uuidUser, email, action) {
        const radios = document.querySelectorAll('input[name="member_bot_action_' + botCleanupSafeText(uuidUser) + '"]');
        if (action) {
            radios.forEach((radio) => {
                radio.checked = String(radio.value || '') === String(action || '');
            });
        }
        if (typeof window.member_patch_apply_bot_review_form === 'function') {
            return window.member_patch_apply_bot_review_form(uuidUser, email);
        }
        return null;
    };

    window.member_patch_clear_bot_review_action = function (uuidUser, email) {
        return window.member_patch_apply_bot_review_action(uuidUser, email, 'clear');
    };

    window.member_patch_apply_bot_review_form = function (uuidUser, email) {
        const key = botCleanupSafeText(uuidUser).trim();
        if (!key) return false;

        const radios = Array.from(document.querySelectorAll('input[name="member_bot_action_' + key + '"]'));
        let action = '';
        radios.forEach((radio) => {
            if (radio && radio.checked && !action) action = botCleanupSafeText(radio.value).trim();
        });
        if (!action) action = 'mark_review';

        const reasonTarget = document.getElementById('txt_member_bot_reason_' + key);
        const reason = reasonTarget ? botCleanupSafeText(reasonTarget.value).trim() : '';

        if (typeof window.click_btn_change_member_bot_status !== 'function') {
            alert(admin_t('members.load_error'));
            return false;
        }

        return window.click_btn_change_member_bot_status(key, action, email || '', reason);
    };

    window.member_patch_resend_bot_verification = function (uuidUser, email) {
        const key = botCleanupSafeText(uuidUser).trim();
        if (!key) return false;
        ...
        return window.click_btn_change_member_bot_status(key, 'resend_verification', email || '', reason);
    };
    ...
    function botCleanupRenderIdentity(member, themeMode, palette, options) {
        ...
        const decisionBadgeText = botCleanupDecisionBadgeText(member);
        ...
        return (
            <div class="flex min-w-0 flex-col gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    {statusLabel ? <p class={botCleanupStatusBadgeClass(member, themeMode)}>{statusLabel}</p> : null}
                    {decisionBadgeText ? <p class={botCleanupDecisionBadgeClass(member, themeMode)}>{decisionBadgeText}</p> : null}
    ...
    function botCleanupRenderEditor(member, themeMode, palette) {
        const key = botCleanupSafeText(member.uuid);
        ...
        const reasonText = botCleanupSafeText(member.bot_decision_reason).trim() || admin_t('members.bot.summary.no_reason');
        const lastActiveText = botCleanupSafeText(member.bot_last_active_signal_at).trim();
        const verificationText = botCleanupSafeText(member.bot_last_verification_sent_at).trim();
        const normalizedStatus = botCleanupNormalizeStatus(member.bot_review_status);
        const selectedReview = normalizedStatus === 'review' || normalizedStatus === '';
        const selectedSuspected = normalizedStatus === 'suspected';

        return (
            <div id={'div_member_bot_editor_' + key} class={(open ? '' : 'hidden ') + 'space-y-4'}>
                <div class={palette.sectionCard}>
                    <div class="flex flex-col gap-4">
                        <div class="space-y-2">
                            <p class={palette.sectionLabel}>{admin_t('members.bot.form.panel_title')}</p>
                            <p class={'text-sm ' + palette.body}>{admin_t('members.bot.form.current_state')}: {currentStateText}</p>
                            <p class={'text-sm ' + palette.muted}>{admin_t('members.bot.form.current_reason')}: {reasonText}</p>
                            {(lastActiveText || verificationText) ? (
                                <div class="flex flex-wrap gap-2">
                                    {lastActiveText ? <p class={palette.metaChip}>{admin_t('members.bot.summary.last_active')}: {lastActiveText}</p> : null}
                                    {verificationText ? <p class={palette.metaChip}>{admin_t('members.bot.summary.last_verification')}: {verificationText}</p> : null}
                                </div>
                            ) : null}
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 xl:grid-cols-5">
                            <div class="xl:col-span-2 space-y-3">
                                <p class={palette.sectionLabel}>{admin_t('members.bot.form.next_status')}</p>
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-1">
                                    <label class="block cursor-pointer">
                                        <input type="radio" name={'member_bot_action_' + key} value="mark_review" class="peer sr-only" defaultChecked={selectedReview} />
                                        <span class={botCleanupRadioCardClass(themeMode)}>{admin_t('members.bot.status.review_queue')}</span>
                                    </label>
                                    <label class="block cursor-pointer">
                                        <input type="radio" name={'member_bot_action_' + key} value="mark_suspected" class="peer sr-only" defaultChecked={selectedSuspected} />
                                        <span class={botCleanupRadioCardClass(themeMode)}>{admin_t('members.bot.status.suspected')}</span>
                                    </label>
                                    <input type="radio" name={'member_bot_action_' + key} value="clear" class="hidden" />
                                </div>
                            </div>
                            <div class="xl:col-span-3 space-y-3">
                                <div class="space-y-2">
                                    ...
                                    <textarea
                                        id={'txt_member_bot_reason_' + key}
                                        rows="4"
                                        defaultValue={botCleanupSafeText(member.bot_decision_reason)}
                                        placeholder={admin_t('members.bot.form.reason_placeholder')}
                                        class={palette.input + ' min-h-[112px] resize-y'}></textarea>
                                </div>
                                <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                                    <button type="button" class={botCleanupPrimaryButtonClass(themeMode)} onClick={() => window.member_patch_apply_bot_review_action(key, member.email)}>
                                        {admin_t('members.bot.form.apply')}
                                    </button>
                                    <button type="button" class={botCleanupGhostButtonClass(themeMode)} onClick={() => window.member_patch_clear_bot_review_action(key, member.email)}>
                                        {admin_t('members.bot.form.clear')}
                                    </button>
                                    {normalizedStatus === 'suspected' ? (
                                        <button type="button" class={botCleanupGhostButtonClass(themeMode)} onClick={() => window.member_patch_resend_bot_verification(key, member.email)}>
                                            {admin_t('members.bot.action.resend')}
                                        </button>
                                    ) : null}
                                </div>
        ...
        return (
            <div class={palette.memberCard}>
                <div class="flex flex-col gap-5">
                        ...
                        <div class="flex flex-col items-start gap-2 xl:items-end">
                            <div class="flex flex-wrap items-center gap-2 xl:justify-end">
                                <button
                                    ...
                                    aria-expanded={showRoleEditor ? 'true' : 'false'}
                                    onClick={() => member_patch_toggle_role_editor(member.uuid)}
                                    class={botCleanupGhostButtonClass(themeMode)}>
                                    {admin_t(showRoleEditor ? 'members.card.hide_role' : 'members.card.change_role')}
                                </button>
                                <button type="button" onClick={() => click_btn_like_change_password(member.email)} class={botCleanupGhostButtonClass(themeMode)}>
                                    {admin_t('members.card.change_password')}
                                </button>
                                <button
                                    id={'btn_member_toggle_bot_editor_' + member.uuid}
                                    type="button"
                                    aria-expanded={showBotEditor ? 'true' : 'false'}
                                    onClick={() => window.member_patch_toggle_bot_review_editor(member.uuid)}
                                    class={botCleanupGhostButtonClass(themeMode)}>
                    ...
                    {botCleanupRenderEditor(member, themeMode, palette)}
