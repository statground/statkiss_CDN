...
function member_patch_parse_period_date(rawLabel, period) {
    const label = String(rawLabel || '')
    if (label === '') {
        return null
    }
    ...
    if (period === 'monthly' || period === 'daily') {
        const parsedMonth = Number(label.slice(5, 7))
        month = Number.isFinite(parsedMonth) && parsedMonth >= 1 ? parsedMonth : 1
    }

    if (period === 'daily') {
        const parsedDay = Number(label.slice(8, 10))
        day = Number.isFinite(parsedDay) && parsedDay >= 1 ? parsedDay : 1
    }

    return new Date(Date.UTC(year, month - 1, day))
}

function member_patch_format_period_key(dateObj, period) {
...
function member_patch_increment_period(dateObj, period) {
...
function member_patch_collect_period_keys(dashboard, period) {
    const rawKeys = []
    const table = ((dashboard || {}).table || {})

    ;['membership', 'addon', 'signup'].forEach(bucketName => {
        const rows = (((table[bucketName] || {})[period]) || [])
        rows.forEach(row => {
    ...
    const startDate = member_patch_parse_period_date(sortedKeys[0], period)
    const endDate = member_patch_parse_period_date(sortedKeys[sortedKeys.length - 1], period)
    ...
    while (cursor <= endDate && guard < 5000) {
        labels.push(member_patch_format_period_key(cursor, period))
        cursor = member_patch_increment_period(cursor, period)
        guard += 1
    }
...
function member_patch_addon_role_order(rows) {
    ...
    const orderedPreferred = preferred.filter(role => discovered.includes(role))
    const extras = discovered.filter(role => !preferred.includes(role)).sort()

    if (orderedPreferred.length === 0 && extras.length === 0) {
...
function member_patch_prepare_role_chart_data(dashboard, period, bucketName) {
    const rows = ((((dashboard || {}).table || {})[bucketName] || {})[period]) || []
    const axisKeys = member_patch_collect_period_keys(dashboard, period)
    const grouped = {}
    ...
    rows.forEach(row => {
        ...
        const normalizedRole = bucketName === 'membership'
            ? member_patch_role_group(roleName)
            : roleName
        ...
        grouped[date][normalizedRole] = member_patch_safe_number(grouped[date][normalizedRole]) + member_patch_safe_number((row || {}).cnt)
    })

    const roleOrder = bucketName === 'membership'
        ? member_patch_role_order()
        : member_patch_addon_role_order(rows)

    const labels = axisKeys.map(label => member_patch_format_period_label(label, period))
    const series = roleOrder.map(role => ({
        name: member_patch_role_label(role),
        data: axisKeys.map(label => member_patch_safe_number((((grouped[label] || {})[role]))))
    }))
...
function member_patch_prepare_membership_chart_data(dashboard, period) {
    return member_patch_prepare_role_chart_data(dashboard, period, 'membership')
}

function member_patch_prepare_addon_chart_data(dashboard, period) {
    return member_patch_prepare_role_chart_data(dashboard, period, 'addon')
}
...
function member_patch_prepare_signup_chart_data(dashboard, period) {
    ...
    rows.forEach(row => {
        ...
        grouped[date] = {
            signup_count: member_patch_safe_number((row || {}).signup_count),
            cumulative_signup_count: member_patch_safe_number((row || {}).cumulative_signup_count)
        }
    })
    ...
    axisKeys.forEach(date => {
        labels.push(member_patch_format_period_label(date, period))
        const row = grouped[date] || {}
        signups.push(member_patch_safe_number(row.signup_count))
        if (Object.prototype.hasOwnProperty.call(row, 'cumulative_signup_count')) {
            lastCumulative = member_patch_safe_number(row.cumulative_signup_count)
        }
        cumulative.push(lastCumulative)
    })

    return { labels, signups, cumulative }
}
...
function member_patch_build_addon_chart_option(dashboard, period, themeMode) {
    const palette = member_patch_get_palette(themeMode)
    const chartData = member_patch_prepare_addon_chart_data(dashboard, period)
    const noDataGraphic = chartData.labels.length === 0
        ? [{
            type: 'text',
            left: 'center',
    ...
    return {
        backgroundColor: 'transparent',
        color: ['#0ea5e9', '#14b8a6', '#8b5cf6', '#f59e0b'],
        animationDuration: 300,
        title: {
            text: admin_t('admin.common.membership_addon'),
            left: 'center',
            top: 18,
            textStyle: {
                color: palette.chart.title,
        ...
        toolbox: {
            ...
            feature: {
                ...
                saveAsImage: {
                    title: 'Download',
                    name: 'statkiss_membership_addon_' + period
                }
            }
        },
        grid: {
...
function Div_member_dashboard_loading(props) {
    ...
    return (
        <div class="flex flex-col w-full space-y-6 animate-pulse">
            ...
            <div class={'w-full rounded-xl border h-[1320px] ' + palette.loadingCard}></div>
...
async function click_btn_download_list() {
    ...
    const url = member_patch_build_url('/admin/ajax_download_member_list/') + '?' + params.toString()
    try {
